...
| `rust_program_ad_ir` | `rust_backed` | `partial` | `partial` | `functional_non_isolated` | non-lowered dynamic indexing semantics, dynamic axes, dynamic trapezoid-grid metadata, dynamic q/method metadata, dynamic ddof/correction metadata, zero-variance std gradients, and broad linalg array adjoints are missing<br>sustained coverage-guided fuzz campaign evidence, Miri, sanitizer, executable registry promotion, LLVM/JIT lowering, provider/hardware evidence, and isolated benchmark evidence are missing |
...
| `differentiable_baseline_scorecard` | `metadata_only` | `not_applicable` | `not_applicable` | `not_applicable` | governance evidence only; no executable Rust surface is required |
