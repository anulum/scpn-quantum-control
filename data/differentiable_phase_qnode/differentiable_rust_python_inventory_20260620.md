...
| `rust_program_ad_ir` | `rust_backed` | `partial` | `partial` | `functional_non_isolated` | non-lowered dynamic indexing semantics, dynamic axes, dynamic q/method metadata, ddof/correction metadata, zero-variance std gradients, and broad linalg array adjoints are missing<br>executable registry promotion, LLVM/JIT lowering, provider/hardware evidence, and isolated benchmark evidence are missing |
