...
| `program_ad_core` | rust_program_ad_ir<br>whole_program_frontend | rust_native_program_ad | non-lowered dynamic indexing semantics, dynamic axes, dynamic trapezoid-grid metadata, dynamic q/method metadata, dynamic ddof/correction metadata, zero-variance std gradients, and broad linalg array adjoints are missing<br>sustained coverage-guided fuzz campaign evidence, Miri, sanitizer, executable registry promotion, LLVM/JIT lowering, provider/hardware evidence, and isolated benchmark evidence are missing<br>static Python source/bytecode metadata has no executable Rust parity requirement<br>compiler lowering remains blocked until a real Program AD backend exists | Round 0 surface integrity and maintainability<br>Round 5 Rustification readiness |
...
| `benchmark_and_claim_governance` | differentiable_baseline_scorecard | benchmark_promotion<br>docs_api_maintainability<br>adoption_licensing | governance evidence only; no executable Rust surface is required | Round 0 surface integrity and maintainability<br>Round 5 Rustification readiness |
