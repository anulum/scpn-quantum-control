...
| ci_benchmark_evidence | SOTA-candidate | diff-qnode-ci-evidence-schema-v1 | diff-qnode-ci-evidence-schema-v1 | GitHub-hosted runners are downgraded to functional_non_isolated.<br>Production isolated_affinity promotion requires a self-hosted isolated-benchmark runner artefact. |
| external_framework_comparison | SOTA-candidate | diff-qnode-external-comparison-schema-v1 | diff-qnode-external-comparison-schema-v1 | Missing framework dependencies are hard_gap rows.<br>Missing LLVM/Enzyme tooling remains dependency_missing until configured. |
