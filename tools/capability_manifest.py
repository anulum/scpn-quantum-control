...
# SCPN Quantum Control — capability manifest tool
...
def build_capability_manifest(
    ...
    return {
        ...
        "counts": {
            ...
            # None (not 0) when the configured directory is absent: a counter
            # over a surface the repo does not have is meaningless, and the
            # renderer omits the row instead of publishing a misleading zero.
            "model_documentation_pages": (
                len(model_docs) if paths.model_docs_root.is_dir() else None
            ),
...
def render_markdown_snapshot(manifest: dict[str, Any]) -> str:
    ...
    # A None count marks a surface the repository does not have (absent
    # configured directory) — omit the row rather than publish a fake zero.
