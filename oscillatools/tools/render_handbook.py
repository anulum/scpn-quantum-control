...
# oscillatools — handbook renderer
"""Render the oscillatools handbook from the live facade and benchmark artefact."""
...
DEFAULT_OUTPUT = REPO_ROOT / "docs" / "handbook.md"
...
def _benchmark_summary(artifact: BenchmarkArtifact) -> list[str]:
    ...
    return [
        ...
        "[Multi-language tier benchmark](tier_benchmarks.md).",
    ]
...
def render_handbook(artifact: BenchmarkArtifact) -> str:
    ...
    lines: list[str] = [
        ...
        "ORCID: 0009-0009-3560-0851",
        "Contact: www.anulum.li | protoscience@anulum.li",
        "Generated by tools/render_handbook.py — do not edit by hand.",
        "-->",
        "",
        "# Handbook",
        "",
        "This handbook is the reference page for `oscillatools`, a differentiable,",
        "control-oriented toolkit for coupled-phase-oscillator (Kuramoto) dynamics. It",
        "is generated from the live `oscillatools` facade and the committed multi-tier",
        "benchmark artefact, so the public API inventory, model families, and",
        ...
        "",
        "The package installs on a NumPy + SciPy floor. The Rust engine (`[rust]`",
        "extra), Julia tier (`[julia]`), JAX and PyTorch backends (`[jax]`, `[torch]`),",
        "and the visualisation and inference helpers (`[viz]`, `[sklearn]`) are optional",
        "accelerators layered over that floor; every documented routine runs on the",
        "pure-Python floor when no accelerator is installed.",
        "",
        "Dense Kuramoto facade hot paths accept NumPy arrays, Python sequences,",
        "and optional Torch or JAX tensors. Tensor inputs are copied to the NumPy",
        "float64 floor for the existing dispatch chain, and array outputs are",
        "restored to the first tensor namespace seen in the call. Scalar observables",
        "remain Python floats, and importing `oscillatools` does not import Torch or",
        "JAX.",
        "",
        "## Mathematical Core",
        "",
        ...
        *_benchmark_table(artifact),
        ...
        "one executable path covering RK4 integration, frequency-locking",
        ...
        "```",
        "",
        "The [example gallery](gallery.md) lists this workflow and how to run it.",
        "Both surfaces use the public `oscillatools` facade and emit diagnostics that",
        "can be compared with this handbook's capability and benchmark tables.",
        "",
        "## Claim Boundaries",
        "",
        ...
        "- The tier artefacts are recorded with the provenance of the run that produced",
        "  them, including the commit and toolchain, so a reader can reproduce the",
        "  measurement rather than take the numbers on trust.",
        "- `oscillatools` is the standalone distribution of the coupled-phase-oscillator",
        "  toolkit. It carries a NumPy + SciPy floor and optional accelerator extras; it",
        "  does not depend on any quantum-provider SDK.",
        "",
    ]
    return "\n".join(lines)
