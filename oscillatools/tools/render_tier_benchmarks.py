...
# oscillatools — side-by-side renderer for the multi-tier benchmark
"""Render the CI-vs-local tier-benchmark comparison from the JSON artefacts.

Consumes the tier-benchmark artefacts recorded for the ``ci`` and ``local``
environments and writes a single Markdown document that places the two side by
side. The document carries:
...
Language policy: pure JSON-to-Markdown formatting with no compute hot loop, so
the Rust-path rule does not apply.
"""
...
def _competitive_framing(ci: dict[str, Any] | None) -> list[str]:
    ...
    return [
        ...
        "",
        "This section is the claim boundary for the tier benchmark. It separates",
        "measured in-package tier competition from unmeasured external package",
        "comparisons and production-latency claims.",
        "",
...
def render(ci: dict[str, Any] | None, local: dict[str, Any] | None) -> str:
    ...
    lines: list[str] = [
        ...
        "-->",
        "# Multi-language tier benchmark — CI vs local",
        "",
        "Per-call **P50** latency (microseconds) of every registered Kuramoto compute",
        "primitive across the Rust, Julia, and Python tiers. The canonical column is",
        "the fixed CI runner (Rust engine built from source); the local column is the",
        "GOTM workstation. Both are reproducible evidence, not published latency claims",
        "— hosted-runner CPU differs from the workstation. A `—` marks a tier not",
        "measured in that run (excluded subset or unavailable binding); see the",
        "per-environment artefacts under `docs/benchmarks/tiers/` for the full",
        "percentile set and parity data.",
        "",
        "## Provenance",
        "",
