...
# A directory inside the working tree, used only as the ``cwd`` for the
# provenance ``git rev-parse`` (which walks up to the enclosing repository).
# Outside a checkout — e.g. an installed wheel — the command fails gracefully
# and the commit is reported as ``unknown``.
_PROVENANCE_ROOT = Path(__file__).resolve().parents[3]
_Command = tuple[str, ...]
...
def _git_commit() -> str:
    ...
    result = _run_admitted_command(("git", "rev-parse", "HEAD"), cwd=_PROVENANCE_ROOT)
    if result is None:
        return "unknown"
