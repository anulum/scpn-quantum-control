...
is integrated by :func:`~oscillatools.accel.kuramoto_delayed.integrate_delayed_kuramoto`, a
...
:mod:`~oscillatools.accel.diff_kuramoto_delayed`. This module adds the JAX counterpart of that
...
* :func:`jax_kuramoto_delayed_gradient` — the reverse-mode gradient of a scalar objective :math:`L`
  ...
  :func:`~oscillatools.accel.diff_kuramoto_delayed.delayed_terminal_value_and_grad` to machine
  ...
  sensitivity lives in :mod:`~oscillatools.accel.diff_kuramoto_delay_sensitivity`.
...
requires JAX (``oscillatools[jax]``) and raises :class:`ImportError` with an install hint when
...
def _load_backend() -> _DelayedBackend:
    ...
    try:
        ...
        raise ImportError(
            "the JAX delayed-Kuramoto tier requires JAX; install oscillatools[jax]"
        ) from error
...
def jax_kuramoto_delayed_trajectory(
    ...
    *,
    ...
    contract to
    :func:`~oscillatools.accel.kuramoto_delayed.integrate_delayed_kuramoto` (its ``phases``
...
def jax_kuramoto_delayed_gradient(
    ...
    *,
    ...
    :func:`~oscillatools.accel.diff_kuramoto_delayed.delayed_terminal_value_and_grad`, here
