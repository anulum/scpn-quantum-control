...
# SCPN Quantum Control — dla_parity.dataset tests
"""Tests for `scpn_quantum_control.dla_parity.dataset`.

Cover:

* Real load of the published DLA-parity dataset (342 circuits, single
...
* Missing run file raises :class:`FileNotFoundError`.
...
def _has_real_data() -> bool:
    return REAL_DATA_DIR.is_dir() and all((REAL_DATA_DIR / f).is_file() for f in RUN_FILES)
...
class TestRealDataset:
    def test_load_full(self) -> None:
        ...
        assert {run.experiment for run in ds.runs} == {
...
class TestIntegrityTamper:
    ...
    def test_tampered_file_without_verify_loads(self, tmp_path: Path) -> None:
        ...
        with tampered.open("w", encoding="utf-8") as fh:
            json.dump(raw, fh)
        ds = load_dla_parity_dataset(data_dir=tmp_path, verify_integrity=False)
...
def _write_fake_dataset(root: Path, run_docs: dict[str, dict[str, object]]) -> None:
...
class TestSchemaValidation:
    ...
    def test_top_level_not_object(self, tmp_path: Path) -> None:
        ...
        for name in RUN_FILES[1:]:
            with (tmp_path / name).open("w", encoding="utf-8") as fh:
                json.dump(_minimal_run(), fh)
