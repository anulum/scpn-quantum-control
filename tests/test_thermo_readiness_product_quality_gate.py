...
"""Lock the BL-19 thermo-readiness quality gate into preflight and CI."""
...
def test_coverage_gate_is_isolated_and_exact() -> None:
    ...
    assert run[-1:] == quality_gates.THERMO_READINESS_PRODUCT_COVERAGE_COHORT
    assert "--fail-under=100" in report
    assert "--include=*/thermo_readiness_product.py" in report
...
def test_ci_runs_and_aggregates_the_thermo_readiness_gate() -> None:
    ...
    assert "--include=*/thermo_readiness_product.py" in block
