...
# SCPN Quantum Control — differentiable-circuit audit quality-gate tests
"""Lock the differentiable-circuit audit quality gate into preflight and CI."""

from pathlib import Path

from tools import diff_contract_audit_quality_gates as quality_gates
...
def test_coverage_gate_is_isolated_and_exact() -> None:
    ...
    run = gates["diff-contract-audit focused coverage"]
    report = gates["diff-contract-audit exact coverage threshold"]
...
def test_preflight_uses_helper_defined_gates() -> None:
    ...
    assert dict(preflight.DIFF_CONTRACT_AUDIT_COVERAGE_GATES) == dict(
        quality_gates.build_coverage_gates(preflight._PY)
    )
    for name, command in quality_gates.build_static_quality_gates(preflight._PY):
        assert dict(preflight.STATIC_GATES)[name] == command
    source = Path("tools/preflight.py").read_text(encoding="utf-8")
    assert "gates.extend(DIFF_CONTRACT_AUDIT_COVERAGE_GATES)" in source


def test_ci_runs_and_aggregates_diff_contract_audit_gate() -> None:
    """Keep the focused CI job and aggregate dependency required."""
    workflow = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    start = workflow.index("  diff-contract-audit-quality:")
