...
class TestStateFidelity:
    ...
    def test_bounded(self) -> None:
        ...
        psi1 = (rng.normal(size=4) + 0j).astype(np.complex128)
        psi1 /= np.linalg.norm(psi1)
        psi2 = (rng.normal(size=4) + 0j).astype(np.complex128)
...
class TestMirrorCircuit:
    def test_perfect_return(self) -> None:
...
class TestXEB:
    def test_perfect_xeb_near_one(self) -> None:
        """No noise → XEB near 1."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
    ...
    def test_cross_entropy_score_uniform(self) -> None:
...
class TestQCVVCertify:
    ...
    def test_simulator_certified(self) -> None:
