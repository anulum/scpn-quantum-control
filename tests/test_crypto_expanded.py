...
# SCPN Quantum Control — Tests for Crypto Expanded
...
class TestNormalizedLaplacianFingerprint:
    ...
    def test_eigenvalues_in_0_2_range(self) -> None:
        fp = normalized_laplacian_fingerprint(K4)
...
class TestTopologyCommitment:
    ...
    def test_different_nonce_different_commitment(self) -> None:
    ...
    def test_verify_rejects_wrong_matrix(self) -> None:
        nonce = b"test"
        c = topology_commitment(K4, nonce)
        K_fake = K4 * 1.01
...
class TestChallengeResponse:
    ...
    def test_wrong_matrix_fails(self) -> None:
        challenge = os.urandom(32)
        response = challenge_response_prove(K4, challenge)
        K_fake = K4 * 0.99
...
class TestFingerprintNoiseTolerance:
    ...
    def test_larger_sigma_more_drift(self) -> None:
...
class TestRowHashFingerprint:
    def test_length_matches_rows(self) -> None:
        hashes = row_hash_fingerprint(K4)
        assert len(hashes) == K4.shape[0]

    def test_verify_row_hash_correct(self) -> None:
        hashes = row_hash_fingerprint(K4)
        for i in range(K4.shape[0]):
            assert verify_row_hash(K4, i, hashes[i])

    def test_verify_row_hash_wrong_row(self) -> None:
        hashes = row_hash_fingerprint(K4)
...
class TestEvolveKeyPhases:
    ...
    def test_phases_evolve(self) -> None:
        theta_0 = np.zeros(4)
...
class TestRotatingKeySchedule:
    ...
    def test_each_window_has_master_and_layers(self) -> None:
        theta_0 = np.zeros(4)
...
class TestGroupKey:
    ...
    def test_different_members_different_key(self) -> None:
        phases = np.array([0.1, 0.2, 0.3, 0.4])
...
class TestRobustnessRandomRemoval:
    ...
    def test_resilience_bounded(self) -> None:
...
class TestRobustnessTargetedRemoval:
    ...
    def test_fraction_bounded(self) -> None:
        result = robustness_targeted_removal(K4)
        assert 0 < result["fraction"] <= 1.0


class TestBestEntanglementPath:
    def test_path_exists_connected_graph(self) -> None:
