...
def test_dense_matrix_falls_back_when_engine_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    """Exercise native, anisotropic, and zero-term fallback boundaries."""
    ...
    anisotropic = knm_to_dense_matrix(k, omega, delta=0.5)
    assert anisotropic.shape == (4, 4)

    zero_operator = knm_to_xxz_hamiltonian(
        np.zeros((1, 1), dtype=np.float64),
        np.zeros(1, dtype=np.float64),
    )
    np.testing.assert_array_equal(zero_operator.to_matrix(), np.zeros((2, 2)))
