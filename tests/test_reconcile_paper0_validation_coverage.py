...
def test_discover_promoted_slices_cover_opening_and_promoted_tail() -> None:
    ...
    assert sum(item.source_record_count for item in slices) == 7238
...
def test_reconcile_promoted_coverage_reports_remaining_middle_gap() -> None:
    ...
    assert result.summary["promoted_record_count"] == 7238
    ...
    assert result.summary["gaps"] == [["None", "P0R06211"]]
