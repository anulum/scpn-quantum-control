...
def test_discover_promoted_slices_cover_opening_and_promoted_tail() -> None:
    ...
    assert sum(item.source_record_count for item in slices) == 4807
...
def test_reconcile_promoted_coverage_reports_remaining_middle_gap() -> None:
    ...
    assert result.summary["promoted_record_count"] == 4807
    ...
    assert result.summary["gaps"] == [["P0R03781", "P0R06211"]]
