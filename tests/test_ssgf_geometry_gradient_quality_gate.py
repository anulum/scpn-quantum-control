...
# SCPN Quantum Control — SSGF geometry-gradient quality-gate tests
"""Lock the SSGF geometry-gradient quality gate into preflight and CI."""

from pathlib import Path

from tools import preflight
from tools import ssgf_geometry_gradient_quality_gates as quality_gates
...
def test_preflight_uses_helper_defined_gates() -> None:
    ...
    assert dict(preflight.SSGF_GEOMETRY_GRADIENT_COVERAGE_GATES) == dict(
        quality_gates.build_coverage_gates(preflight._PY)
    )
    for name, command in quality_gates.build_static_quality_gates(preflight._PY):
        assert dict(preflight.STATIC_GATES)[name] == command
    source = Path("tools/preflight.py").read_text(encoding="utf-8")
    assert "gates.extend(SSGF_GEOMETRY_GRADIENT_COVERAGE_GATES)" in source


def test_ci_runs_and_aggregates_ssgf_geometry_gradient_gate() -> None:
    """Keep the focused CI job and aggregate dependency required."""
    workflow = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    start = workflow.index("  ssgf-geometry-gradient-quality:")
