...
# SCPN Quantum Control — compiler-boundary quality-gate tests
"""Lock the BL-19 compiler-boundary product gate into preflight and CI."""

from pathlib import Path

from tools import compiler_boundary_product_quality_gates as quality_gates
...
def test_preflight_uses_helper_defined_gates() -> None:
    ...
    assert dict(preflight.COMPILER_BOUNDARY_PRODUCT_COVERAGE_GATES) == dict(
...
def test_ci_runs_and_aggregates_gate() -> None:
    ...
    start = workflow.index("  compiler-boundary-product-quality:")
    end = workflow.index("\n\n  decisive-advantage-quality:", start)
    block = workflow[start:end]
    assert all(path in block for path in quality_gates.COMPILER_BOUNDARY_PRODUCT_QUALITY_RATCHET)
    assert "compiler-boundary-product-quality" in workflow[workflow.index("  ci-gate:") :]
