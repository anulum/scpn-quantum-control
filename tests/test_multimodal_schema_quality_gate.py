...
def test_helper_builds_strict_preview_and_exact_gates() -> None:
    ...
    run = coverage["multimodal-schema focused coverage"]
    report = coverage["multimodal-schema exact coverage threshold"]
...
def test_preflight_uses_helper_commands_verbatim() -> None:
    ...
    assert dict(preflight.MULTIMODAL_SCHEMA_COVERAGE_GATES) == dict(
...
def test_ci_runs_and_aggregates_multimodal_schema() -> None:
    ...
    for path in quality_gates.MULTIMODAL_SCHEMA_QUALITY_RATCHET:
        assert path in block
    for path in quality_gates.MULTIMODAL_SCHEMA_COVERAGE_COHORT:
        assert path in block
    assert quality_gates.MULTIMODAL_SCHEMA_COVERAGE_INCLUDE in block
    consumer_start = workflow.index("  qrc-baseline-quality:", end)
    consumer_block = workflow[
        consumer_start : workflow.index("\n\n  josephson-magnitude-study-quality:", consumer_start)
    ]
    assert "needs: [lint, multimodal-schema-quality]" in consumer_block
