...
class TestGraphGenerators:
    def test_erdos_renyi_symmetric(self) -> None:
        K = _erdos_renyi_coupling(16, 0.5)
        np.testing.assert_allclose(K, K.T, atol=1e-12)

    def test_erdos_renyi_density(self) -> None:
        K = _erdos_renyi_coupling(16, 1.0)
        assert np.all(K[np.eye(16) == 0] > 0)

    def test_erdos_renyi_empty(self) -> None:
    ...
    def test_watts_strogatz_shape(self) -> None:
        K = _watts_strogatz_coupling(16, k=4, beta=0.3)
        assert K.shape == (16, 16)

    def test_watts_strogatz_symmetric(self) -> None:
...
class TestGraphInvariants:
    def test_ring_zero_diagonal(self) -> None:
        K = _ring_coupling(8, k=1)
        np.testing.assert_allclose(np.diag(K), 0.0)

    def test_erdos_renyi_zero_diagonal(self) -> None:
        K = _erdos_renyi_coupling(8, 0.5)
        np.testing.assert_allclose(np.diag(K), 0.0)

    def test_watts_strogatz_zero_diagonal(self) -> None:
        K = _watts_strogatz_coupling(8, k=2, beta=0.3)
        np.testing.assert_allclose(np.diag(K), 0.0)

    def test_ring_non_negative(self) -> None:
        K = _ring_coupling(16, k=2)
        assert np.all(K >= 0)

    def test_erdos_renyi_non_negative(self) -> None:
...
class TestGraphPipeline:
    def test_ring_to_hamiltonian(self) -> None:
    ...
    def test_erdos_renyi_to_hamiltonian(self) -> None:
        from scpn_quantum_control.bridge.knm_hamiltonian import (
            OMEGA_N_16,
            knm_to_hamiltonian,
        )
...
def _mock_compute_persistence(
    theta: NDArray[np.float64], persistence_threshold: float = 0.1
) -> PersistenceResult:
...
class TestMeasurePH1AtTransition:
    ...
    @_MOCK_CP
    def test_returns_mean_and_std(self, _mock: MagicMock) -> None:
    ...
    @_MOCK_CP
    def test_reproducible(self, _mock: MagicMock) -> None:
        K = _ring_coupling(8, k=2)
    ...
    @_MOCK_CP
    def test_zero_coupling(self, _mock: MagicMock) -> None:
    ...
    @_MOCK_CP
    def test_erdos_renyi_input(self, _mock: MagicMock) -> None:
        K = _erdos_renyi_coupling(8, 0.5)
        mean, std = _measure_p_h1_at_transition(K, n_thermalize=10, n_samples=5, seed=42)
        assert 0 <= mean <= 1

    @_MOCK_CP
    def test_watts_strogatz_input(self, _mock: MagicMock) -> None:
        K = _watts_strogatz_coupling(8, k=4, beta=0.3)
        mean, std = _measure_p_h1_at_transition(K, n_thermalize=10, n_samples=5, seed=42)
        assert 0 <= mean <= 1

    @_MOCK_CP
    def test_measurement_samples_collected(self, _mock: MagicMock) -> None:
...
def _run_scan(**kwargs: Any) -> list[GraphP_H1_Result]:
    """Run scan_graph_topologies with both mocks active.

    Both names are module globals of ``graph_topology_scan``: ``_RIPSER_AVAILABLE``
    is read there through its own import, so the module dict — not the defining
    module ``persistent_homology`` — is the binding the scan actually consults.
    """
    import scpn_quantum_control.analysis.graph_topology_scan as _mod

    module_globals = vars(_mod)
    orig_measure = module_globals["_measure_p_h1_at_transition"]
    orig_ripser = module_globals["_RIPSER_AVAILABLE"]
    module_globals.update(_measure_p_h1_at_transition=_fast_measure_p_h1, _RIPSER_AVAILABLE=True)
    try:
        return scan_graph_topologies(**kwargs)
    finally:
        module_globals.update(
            _measure_p_h1_at_transition=orig_measure, _RIPSER_AVAILABLE=orig_ripser
        )
...
class TestScanGraphTopologies:
    ...
    def test_erdos_renyi_parameters(self) -> None:
    ...
    def test_knm_complete_uses_paper27(self) -> None:
    ...
    def test_raises_without_ripser(self) -> None:
        with (
            patch(
                "scpn_quantum_control.analysis.graph_topology_scan._RIPSER_AVAILABLE",
                False,
...
class TestWattsStrogatzDenseGraph:
    ...
    def test_complete_graph_no_candidates(self) -> None:
    ...
    def test_nearly_complete(self) -> None:
...
class TestMeasurePH1PythonFallback:
    ...
    @_MOCK_CP
    def test_python_fallback_no_rust(self, _mock: MagicMock) -> None:
    ...
    @_MOCK_CP
    def test_python_fallback_finds_best_temperature(self, _mock: MagicMock) -> None:
