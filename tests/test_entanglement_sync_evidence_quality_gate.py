...
# SCPN Quantum Control — entanglement-sync evidence quality-gate tests
"""Lock the entanglement-sync evidence gate into preflight and CI."""

from pathlib import Path

from tools import entanglement_sync_evidence_quality_gates as quality_gates
...
def test_preflight_uses_helper_defined_gates() -> None:
    ...
    assert dict(preflight.ENTANGLEMENT_SYNC_EVIDENCE_COVERAGE_GATES) == dict(
        quality_gates.build_coverage_gates(preflight._PY)
    )
    for name, command in quality_gates.build_static_quality_gates(preflight._PY):
        assert dict(preflight.STATIC_GATES)[name] == command
    source = Path("tools/preflight.py").read_text(encoding="utf-8")
    assert "gates.extend(ENTANGLEMENT_SYNC_EVIDENCE_COVERAGE_GATES)" in source


def test_ci_runs_and_aggregates_evidence_gate() -> None:
    """Keep the focused CI job and aggregate dependency required."""
    workflow = read_ci_workflow_source()
    start = workflow.index("  entanglement-sync-evidence-quality:")
