...
def test_static_gate_is_strict_and_numpy_documented() -> None:
    ...
    submission_source = "src/scpn_quantum_control/hardware/feedback_submission.py"
    submission_tests = {
        "tests/test_feedback_submission.py",
        "tests/test_feedback_submission_branches.py",
    }
    assert submission_source in quality_gates.HARDWARE_SAFE_EXECUTION_TYPING_RATCHET
    assert submission_source in quality_gates.HARDWARE_SAFE_EXECUTION_QUALITY_RATCHET
    assert submission_tests.issubset(quality_gates.HARDWARE_SAFE_EXECUTION_QUALITY_RATCHET)
    assert submission_tests.issubset(quality_gates.HARDWARE_SAFE_EXECUTION_COVERAGE_COHORT)
    assert "*/hardware/feedback_submission.py" in (
        quality_gates.HARDWARE_SAFE_EXECUTION_COVERAGE_INCLUDE
    )
    dossier_source = "src/scpn_quantum_control/hardware/job_dossier.py"
    dossier_tests = {"tests/test_job_dossier.py", "tests/test_job_dossier_branches.py"}
    assert dossier_source in quality_gates.HARDWARE_SAFE_EXECUTION_TYPING_RATCHET
    assert dossier_source in quality_gates.HARDWARE_SAFE_EXECUTION_QUALITY_RATCHET
    ...
    s1_source = "src/scpn_quantum_control/hardware/s1_feedback_ibm.py"
    s1_tests = {
        "tests/test_s1_feedback_ibm.py",
        "tests/test_s1_feedback_ibm_branches.py",
    }
    assert s1_source in quality_gates.HARDWARE_SAFE_EXECUTION_TYPING_RATCHET
    assert s1_source in quality_gates.HARDWARE_SAFE_EXECUTION_QUALITY_RATCHET
    assert s1_tests.issubset(quality_gates.HARDWARE_SAFE_EXECUTION_TYPING_RATCHET)
    assert s1_tests.issubset(quality_gates.HARDWARE_SAFE_EXECUTION_QUALITY_RATCHET)
    assert s1_tests.issubset(quality_gates.HARDWARE_SAFE_EXECUTION_COVERAGE_COHORT)
    assert "*/hardware/s1_feedback_ibm.py" in (
        quality_gates.HARDWARE_SAFE_EXECUTION_COVERAGE_INCLUDE
    )
