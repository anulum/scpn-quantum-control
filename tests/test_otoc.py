# SPDX-License-Identifier: AGPL-3.0-or-later
# Commercial license available
# © Concepts 1996–2026 Miroslav Šotek. All rights reserved.
# © Code 2020–2026 Miroslav Šotek. All rights reserved.
# ORCID: 0009-0009-3560-0851
# Contact: www.anulum.li | protoscience@anulum.li
# SCPN Quantum Control — Tests for Otoc
"""Tests for OTOC measurement protocol."""

from __future__ import annotations

import builtins
from typing import Any

import numpy as np
import pytest

import scpn_quantum_control.analysis.otoc as otoc_module
from scpn_quantum_control.analysis.otoc import (
    OTOC,
    OTOCResult,
    _estimate_scrambling_time,
    _pauli_matrix,
    compute_otoc,
)
from scpn_quantum_control.bridge.knm_hamiltonian import (
    OMEGA_N_16,
    build_knm_paper27,
    knm_to_dense_matrix,
)
from scpn_quantum_control.dense_budget import DenseAllocationError


class TestPauliMatrix:
    """Verify embedded single-qubit Pauli construction."""

    def test_x_hermitian(self) -> None:
        """Construct a Hermitian embedded Pauli-X operator."""
        X = _pauli_matrix("X", 0, 2)
        np.testing.assert_allclose(X, X.conj().T, atol=1e-12)

    def test_z_diagonal(self) -> None:
        """Preserve the expected Pauli-Z diagonal signs."""
        Z = _pauli_matrix("Z", 0, 2)
        assert Z[0, 0] == pytest.approx(1.0)
        assert Z[3, 3] == pytest.approx(-1.0)

    def test_shape(self) -> None:
        """Return the full Hilbert-space matrix shape."""
        P = _pauli_matrix("Y", 1, 3)
        assert P.shape == (8, 8)

    def test_unitary(self) -> None:
        """Square the embedded Pauli-X operator to identity."""
        X = _pauli_matrix("X", 0, 2)
        np.testing.assert_allclose(X @ X, np.eye(4), atol=1e-12)


class TestComputeOTOC:
    """Verify the public exact OTOC function and its boundaries."""

    def test_returns_result(self) -> None:
        """Return the typed OTOC result envelope."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        result = compute_otoc(K, omega, times=np.linspace(0, 1, 5, dtype=np.float64))
        assert isinstance(result, OTOCResult)

    def test_otoc_at_t0_is_one(self) -> None:
        """F(0) = <W†VWV†> with W,V Pauli → should be ±1."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        times = np.array([0.0, 0.1, 0.2], dtype=np.float64)
        result = compute_otoc(K, omega, times=times)
        assert abs(result.otoc_values[0]) == pytest.approx(1.0, abs=0.01)

    def test_otoc_bounded(self) -> None:
        """Keep every exact Pauli OTOC value within unit magnitude."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        result = compute_otoc(K, omega, times=np.linspace(0, 1, 10, dtype=np.float64))
        for val in result.otoc_values:
            assert abs(val) <= 1.0 + 1e-10

    def test_times_shape(self) -> None:
        """Align the returned series with the supplied time grid."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        t = np.linspace(0, 2, 15, dtype=np.float64)
        result = compute_otoc(K, omega, times=t)
        assert result.otoc_values.shape == (15,)

    def test_n_qubits(self) -> None:
        """Record the evaluated Hilbert-space qubit count."""
        K = build_knm_paper27(L=4)
        omega = OMEGA_N_16[:4]
        result = compute_otoc(K, omega, times=np.array([0.0, 0.5], dtype=np.float64))
        assert result.n_qubits == 4

    def test_operator_labels(self) -> None:
        """Record the selected operator qubits in result provenance."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        result = compute_otoc(
            K,
            omega,
            times=np.array([0.0], dtype=np.float64),
            w_qubit=0,
            v_qubit=2,
        )
        assert "Z_0" in result.operator_w
        assert "X_2" in result.operator_v

    def test_custom_paulis(self) -> None:
        """Record caller-selected Pauli labels in result provenance."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        result = compute_otoc(
            K,
            omega,
            times=np.array([0.0, 0.5], dtype=np.float64),
            w_pauli="X",
            v_pauli="Y",
        )
        assert "X" in result.operator_w
        assert "Y" in result.operator_v

    def test_scrambling_time_type(self) -> None:
        """Return either no threshold crossing or a floating-point time."""
        K = build_knm_paper27(L=3)
        omega = OMEGA_N_16[:3]
        result = compute_otoc(K, omega, times=np.linspace(0, 5, 20, dtype=np.float64))
        # May or may not find scrambling time depending on dynamics
        assert result.scrambling_time is None or isinstance(result.scrambling_time, float)

    def test_scpn_otoc_measurement(self) -> None:
        """Record OTOC at SCPN default parameters."""
        K = build_knm_paper27(L=4)
        omega = OMEGA_N_16[:4]
        result = compute_otoc(K, omega, times=np.linspace(0, 2, 10, dtype=np.float64))
        print(f"\n  OTOC (4 osc): F(0)={result.otoc_values[0]:.4f}")
        print(f"  F(t_max)={result.otoc_values[-1]:.4f}")
        print(f"  Lyapunov estimate: {result.lyapunov_estimate}")
        print(f"  Scrambling time: {result.scrambling_time}")
        assert isinstance(result.otoc_values[0], float)

    def test_rejects_dense_workspace_budget_before_hamiltonian_build(self) -> None:
        """Reject an over-budget request before Hamiltonian construction."""
        K = build_knm_paper27(L=4)
        omega = OMEGA_N_16[:4]

        with pytest.raises(DenseAllocationError, match="OTOC exact dense workspace"):
            compute_otoc(
                K,
                omega,
                times=np.array([0.0], dtype=np.float64),
                max_dense_gib=1e-12,
            )


class TestOTOCClassInterface:
    """Verify the campaign-compatible class facade."""

    def test_compute_delegates_to_function(self) -> None:
        """Delegate configured operators and budgets to the public function."""
        K = build_knm_paper27(L=2)
        omega = OMEGA_N_16[:2]
        times = np.array([0.0, 0.2], dtype=np.float64)

        otoc = OTOC(
            K,
            omega,
            w_qubit=0,
            v_qubit=1,
            w_pauli="X",
            v_pauli="Z",
            max_dense_gib=1.0,
        )
        result = otoc.compute(times=times)

        assert isinstance(result, OTOCResult)
        np.testing.assert_allclose(result.times, times)
        assert result.operator_w == "X_0"
        assert result.operator_v == "Z_1"

    def test_compute_requires_k_and_omega(self) -> None:
        """Refuse class execution without both Hamiltonian inputs."""
        with pytest.raises(ValueError, match="requires K and omega"):
            OTOC().compute(times=np.array([0.0], dtype=np.float64))


# ---------------------------------------------------------------------------
# Coverage: Python fallback, edge cases
# ---------------------------------------------------------------------------


class TestOTOCCoverage:
    """Cover missing lines: Python fallback, Lyapunov/scrambling None paths."""

    def test_default_times(self) -> None:
        """Cover line 101: times=None defaults to linspace(0, 2, 30)."""
        K = build_knm_paper27(L=2)
        omega = OMEGA_N_16[:2]
        result = compute_otoc(K, omega, times=None)
        assert len(result.times) == 30

    def test_python_fallback(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Cover lines 133-143: Python OTOC when Rust unavailable."""
        K = build_knm_paper27(L=2)
        omega = OMEGA_N_16[:2]
        times = np.linspace(0, 0.5, 5, dtype=np.float64)

        real_import = builtins.__import__
        H_mat = knm_to_dense_matrix(K, omega)

        def import_without_engine(name: str, *args: Any, **kwargs: Any) -> Any:
            if name == "scpn_quantum_engine":
                raise ImportError("forced Python OTOC path")
            return real_import(name, *args, **kwargs)

        monkeypatch.setattr(otoc_module, "knm_to_dense_matrix", lambda *_args, **_kwargs: H_mat)
        monkeypatch.setattr(builtins, "__import__", import_without_engine)
        result = compute_otoc(K, omega, times=times)

        assert len(result.otoc_values) == 5
        assert np.isfinite(result.otoc_values).all()

    def test_lyapunov_none_zero_f0(self) -> None:
        """Cover line 167: F(0) ≈ 0 → Lyapunov returns None."""
        from scpn_quantum_control.analysis.otoc import _estimate_lyapunov

        times = np.linspace(0, 1, 10, dtype=np.float64)
        otoc = np.zeros(10, dtype=np.float64)
        result = _estimate_lyapunov(times, otoc)
        assert result is None

    def test_lyapunov_none_too_few_decay_points(self) -> None:
        """Cover line 178: fewer than 3 positive decay points."""
        from scpn_quantum_control.analysis.otoc import _estimate_lyapunov

        times = np.linspace(0, 1, 10, dtype=np.float64)
        otoc = np.ones(10, dtype=np.float64) * 0.99
        result = _estimate_lyapunov(times, otoc)
        assert result is None

    def test_scrambling_none_zero_f0(self) -> None:
        """Cover line 191: F(0) ≈ 0 → scrambling returns None."""
        from scpn_quantum_control.analysis.otoc import _estimate_scrambling_time

        times = np.linspace(0, 1, 10, dtype=np.float64)
        otoc = np.zeros(10, dtype=np.float64)
        result = _estimate_scrambling_time(times, otoc)
        assert result is None

    def test_scrambling_none_no_threshold(self) -> None:
        """Cover line 197: OTOC never drops below 1/e → None."""
        from scpn_quantum_control.analysis.otoc import _estimate_scrambling_time

        times = np.linspace(0, 1, 10, dtype=np.float64)
        otoc = np.ones(10, dtype=np.float64)
        result = _estimate_scrambling_time(times, otoc)
        assert result is None

    def test_scrambling_time_first_threshold_crossing(self) -> None:
        """Return the first time below the one-over-e threshold."""
        times = np.array([0.0, 0.25, 0.5, 0.75], dtype=np.float64)
        otoc = np.array([1.0, 0.8, 0.2, 0.1], dtype=np.float64)

        assert _estimate_scrambling_time(times, otoc) == 0.5
