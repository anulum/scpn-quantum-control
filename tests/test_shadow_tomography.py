...
def _zero_state(n: int) -> NDArray[np.complex128]:
    psi = np.zeros(2**n, dtype=np.complex128)
    psi[0] = 1.0
    return psi


def _plus_state(n: int) -> NDArray[np.complex128]:
    """Equal superposition |+>^n."""
    psi: NDArray[np.complex128] = np.ones(2**n, dtype=np.complex128) / np.sqrt(2**n)
    return psi


class TestPauliFromLabel:
    """Verify Pauli-word matrix construction."""

    def test_identity(self) -> None:
        """Construct the two-qubit identity word."""
        P = _pauli_from_label("II", 2)
        np.testing.assert_allclose(P, np.eye(4), atol=1e-12)

    def test_z_single(self) -> None:
        """Construct the signed diagonal single-qubit Z matrix."""
        P = _pauli_from_label("Z", 1)
        assert P[0, 0] == pytest.approx(1.0)
        assert P[1, 1] == pytest.approx(-1.0)

    def test_shape(self) -> None:
        """Scale the matrix shape with the Pauli-word length."""
        P = _pauli_from_label("XYZ", 3)
        assert P.shape == (8, 8)


class TestEstimatePauliExpectation:
    """Verify direct Pauli expectation estimation."""

    def test_z_on_zero_state_positive(self) -> None:
    ...
    def test_returns_float(self) -> None:
        """Return a scalar estimate for a mixed identity/Pauli word."""
        psi = _zero_state(2)
...
class TestClassicalShadowEstimation:
    """Verify the public multi-observable shadow pipeline."""
    ...
    def test_all_observables_estimated(self) -> None:
        """Retain every requested observable name in the output."""
        psi = _zero_state(2)
        obs = {"Z0": "ZI", "Z1": "IZ", "XX": "XX"}
        result = classical_shadow_estimation(psi, 2, obs, n_shots=100, seed=42)
        assert set(result.estimated_observables.keys()) == {"Z0", "Z1", "XX"}

    def test_shadow_norm_bound_positive(self) -> None:
        """Report a positive finite-shot shadow-norm bound."""
        psi = _zero_state(2)
        obs = {"ZZ": "ZZ"}
        result = classical_shadow_estimation(psi, 2, obs, n_shots=500, seed=42)
        assert result.shadow_norm_bound > 0

    def test_n_qubits_and_shots(self) -> None:
        """Preserve the requested qubit and shot counts."""
        psi = _zero_state(3)
        obs = {"Z0": "ZII"}
    ...
    def test_plus_state_z_near_zero(self) -> None:
        """<+|Z|+> = 0."""
        psi = _plus_state(1)
        obs = {"Z": "Z"}
...
class TestCliffordGroup:
    """Verify the generated single-qubit Clifford group."""

    def test_exactly_24_elements(self) -> None:
    ...
    def test_closed_under_multiplication(self) -> None:
        """Product of any two Cliffords must be in the group (up to global phase)."""

        def in_group(M: NDArray[np.complex128]) -> bool:
    ...
    def test_contains_h_and_s(self) -> None:
