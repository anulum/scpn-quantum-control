...
# SCPN Quantum Control — stochastic-estimators quality-gate tests
"""Lock the BL-19 stochastic-estimators product gate into preflight and CI."""

from pathlib import Path

from tools import preflight
from tools import stochastic_estimators_product_quality_gates as quality_gates
...
def test_static_gate_is_strict_and_numpy_documented() -> None:
    ...
    assert (
        gates["mypy-strict-stochastic-estimators-product-quality"][5:]
        == quality_gates.STOCHASTIC_ESTIMATORS_PRODUCT_QUALITY_RATCHET
    )
    assert "D,D413" in gates["ruff D stochastic-estimators-product quality ratchet"]
...
def test_preflight_uses_helper_defined_gates() -> None:
    ...
    assert dict(preflight.STOCHASTIC_ESTIMATORS_PRODUCT_COVERAGE_GATES) == dict(
...
def test_ci_runs_and_aggregates_gate() -> None:
    ...
    start = workflow.index("  stochastic-estimators-product-quality:")
