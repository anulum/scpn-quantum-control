...
def test_fidelity_at_zero_depth() -> None:
    """Return unity before any gate or readout errors accrue."""
    assert fidelity_at_depth(0, 4) == 1.0


def test_fidelity_decreases_with_depth() -> None:
    """Reduce the estimate monotonically as circuit depth grows."""
...
def test_fidelity_decreases_with_qubits() -> None:
    """Reduce the estimate as more qubits accumulate errors."""
...
def test_fidelity_bounded_zero_one() -> None:
    """Keep representative fidelity estimates within probability bounds."""
    for depth in [1, 10, 100, 500, 1000]:
        f = fidelity_at_depth(depth, 4)
        assert 0.0 <= f <= 1.0


def test_negative_depth_raises() -> None:
    """Reject a negative circuit depth before evaluating the model."""
    with pytest.raises(ValueError, match="non-negative"):
        fidelity_at_depth(-1, 4)


def test_zero_qubits_raises() -> None:
    """Reject a fidelity estimate for an empty circuit."""
    with pytest.raises(ValueError, match="n_qubits"):
        fidelity_at_depth(10, 0)


def test_coherence_budget_returns_valid_depth() -> None:
    """Return a depth whose estimated fidelity meets the threshold."""
    result = coherence_budget(4, fidelity_threshold=0.5)
    assert result["max_depth"] >= 0
    assert result["fidelity_at_max"] >= 0.5


def test_budget_depth_decreases_with_qubits() -> None:
    """Reduce the available depth as the circuit width grows."""
...
def test_budget_depth_decreases_with_higher_threshold() -> None:
    """Reduce the available depth for a stricter fidelity threshold."""
...
def test_budget_fidelity_curve_populated() -> None:
    """Return bounded estimates at the documented sample depths."""
    result = coherence_budget(4, fidelity_threshold=0.5)
...
def test_budget_hardware_params_present() -> None:
    """Expose the hardware inputs used by the estimate."""
...
def test_invalid_threshold_raises() -> None:
    """Reject both endpoints of the open threshold interval."""
...
def test_invalid_budget_qubits_raises() -> None:
    """Reject a coherence budget for an empty circuit."""
    with pytest.raises(ValueError, match="n_qubits"):
        coherence_budget(0)


def test_worse_hardware_gives_shorter_budget() -> None:
    """Reduce the budget when the two-qubit error rate increases."""
