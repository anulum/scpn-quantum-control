...
# SCPN Quantum Control — stable core preflight fixtures gate tests
# scpn-quantum-control -- stable core preflight fixture tests
"""Tests for stable-core preflight fixture comparison and gate helper wiring."""
...
from scripts.compare_stable_core_preflight_fixtures import (
    compare_stable_core_preflight_fixtures,
    write_stable_core_preflight_fixtures,
)
from scripts.run_stable_core_preflight_gate import (
    COMPARATOR_SCRIPT,
    build_stable_core_preflight_gate_commands,
)


def _make_expected_payload(tmp_path: Path) -> tuple[Path, Path]:
    """Write committed-equivalent preflight artifacts into temporary paths."""

    expected_json = tmp_path / "expected_stable_core_preflight_fixtures.json"
    expected_markdown = tmp_path / "expected_stable_core_preflight_fixtures.md"
    write_stable_core_preflight_fixtures(
...
def test_compare_returns_valid_for_matching_expected_artifacts(tmp_path: Path) -> None:
    """Generated stable-core preflight fixtures match an equivalent committed baseline."""

    expected_json, expected_markdown = _make_expected_payload(tmp_path)

    result = compare_stable_core_preflight_fixtures(
...
def test_gate_helper_builds_comparator_command_with_explicit_expected_paths(
    ...
    commands = build_stable_core_preflight_gate_commands(
    ...
    assert commands == (
        (
            sys.executable,
            "scripts/compare_stable_core_preflight_fixtures.py",
            "--expected-json",
            str(expected_json),
            "--expected-markdown",
