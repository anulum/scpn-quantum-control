...
# SCPN Quantum Control — migration-guides quality-gate tests
"""Lock the BL-19 migration-guides product gate into preflight and CI."""

from pathlib import Path

from tools import migration_guides_product_quality_gates as quality_gates
...
def test_static_gate_is_strict_and_numpy_documented() -> None:
    ...
    assert (
        gates["mypy-strict-migration-guides-product-quality"][5:]
        == quality_gates.MIGRATION_GUIDES_PRODUCT_QUALITY_RATCHET
    )
    assert "D,D413" in gates["ruff D migration-guides-product quality ratchet"]
...
def test_preflight_uses_helper_defined_gates() -> None:
    ...
    assert dict(preflight.MIGRATION_GUIDES_PRODUCT_COVERAGE_GATES) == dict(
...
def test_ci_runs_and_aggregates_gate() -> None:
    ...
    start = workflow.index("  migration-guides-product-quality:")
