...
def test_discretize_shapes() -> None:
...
def test_laplacian_symmetric() -> None:
    solver = VQLS_GradShafranov(n_qubits=3)
    A, _ = solver.discretize()
    np.testing.assert_allclose(A, A.T, atol=1e-12)


def test_laplacian_positive_definite() -> None:
    solver = VQLS_GradShafranov(n_qubits=3)
    A, _ = solver.discretize()
...
def test_solve_returns_array() -> None:
    solver = VQLS_GradShafranov(n_qubits=3)
...
def test_solve_respects_boundary_conditions() -> None:
...
def test_solve_before_build_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """RuntimeError guard fires when discretize is sabotaged."""
    solver = VQLS_GradShafranov(n_qubits=2)
    monkeypatch.setattr(solver, "discretize", lambda: None)  # break auto-build
    with pytest.raises(RuntimeError, match="discretize"):
        solver.solve()


def test_solve_seeded_deterministic() -> None:
...
def test_source_width_affects_solution() -> None:
...
def test_ansatz_params_increase_with_reps() -> None:
    solver = VQLS_GradShafranov(n_qubits=3)
...
def test_grid_size_matches_n_qubits() -> None:
    for n in (2, 3, 4):
        s = VQLS_GradShafranov(n_qubits=n)
        assert s.grid_size == 2**n


def test_pipeline_solve_to_residual() -> None:
...
def test_solve_matches_direct_grad_shafranov_reference(n_qubits: int) -> None:
...
def test_solve_diagnostics_expose_residual_repair_for_default_grid() -> None:
...
def test_solve_fail_closed_without_residual_repair() -> None:
...
def test_solve_runs_configured_multi_restart_count(monkeypatch: pytest.MonkeyPatch) -> None:
    solver = VQLS_GradShafranov(n_qubits=2)
    call_count = 0
...
def test_zero_operator_uses_unit_cost_guard(monkeypatch: pytest.MonkeyPatch) -> None:
    solver = VQLS_GradShafranov(n_qubits=2)
...
def test_imaginary_state_raises_clear_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class ComplexState:
        def __array__(self, dtype: Any = None, copy: bool | None = None) -> NDArray[np.complex128]:
    ...
    monkeypatch.setattr(vqls_gs, "minimize", fake_minimise)
    monkeypatch.setattr(Statevector, "from_instruction", lambda _circuit: ComplexState())
