...
# SCPN Quantum Control — differentiable gradient descent optimizer tests
# scpn-quantum-control -- gradient-descent optimizer differentiable tests
"""Regression tests for gradient-descent optimizer contracts."""
...
def test_gradient_descent_step_projects_box_bounds() -> None:
    ...
    _assert_allclose(updated, [-0.5, 0.25])
...
def test_gradient_descent_step_wraps_periodic_bounds() -> None:
    ...
    _assert_allclose(updated, [0.25], atol=1.0e-12)
...
def test_gradient_descent_step_clips_trainable_gradient_norm() -> None:
    ...
    _assert_allclose(updated, [-0.6, -0.8, 0.0], atol=1.0e-12)
...
def test_parameter_bounds_reject_invalid_intervals() -> None:
    ...
    with pytest.raises(ValueError, match="periodic flag"):
        ParameterBounds(lower=-math.pi, upper=math.pi, periodic=cast(Any, np.bool_(True)))
...
def test_optimizer_minimize_converges_for_shift_compatible_quadratic() -> None:
    ...
    _assert_allclose(result.values, [0.0], atol=1.0e-6)
    _assert_allclose(result.best_values, result.values)
...
def test_optimizer_minimize_supports_finite_difference_backend() -> None:
    ...
    _assert_allclose(result.values, [0.0], atol=1.0e-5)
...
def test_optimizer_result_tracks_best_iterate_when_final_worsens() -> None:
    ...
    _assert_allclose(result.best_values, [0.0])
...
def test_optimizer_minimize_rejects_invalid_loop_controls() -> None:
    ...
    with pytest.raises(ValueError, match="max_steps"):
        optimizer.minimize(lambda values: math.sin(values[0]), [0.1], max_steps=cast(Any, True))
...
def test_gradient_result_rejects_malformed_metadata() -> None:
    ...
    with pytest.raises(ValueError, match="trainable mask must contain booleans"):
        GradientResult(
            ...
            evaluations=3,
            parameter_names=("a",),
            trainable=cast(Any, (np.bool_(True),)),
        )
