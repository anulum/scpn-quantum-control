...
def test_key_rate_moderate_concurrence() -> None:
...
def test_key_rate_clamped_entropy_low() -> None:
...
def test_key_rate_high_concurrence() -> None:
...
def test_robustness_targeted_fully_connected() -> None:
...
def test_robustness_targeted_chain_falls_through() -> None:
...
def test_best_path_revisit_skip() -> None:
...
def test_best_path_boundary_validation() -> None:
...
def test_best_path_same_source_target() -> None:
    K = np.array([[0.0, 0.5], [0.5, 0.0]])
    result = best_entanglement_path(K, source=0, target=0)
    assert result["path"] == [0]


def test_best_path_direct_connection() -> None:
...
def test_best_path_3_nodes() -> None:
...
def test_active_channel_graph_4_nodes() -> None:
