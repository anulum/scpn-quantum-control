...
"""Tests for Cirq backend adapter — multi-angle coverage."""
...
class TestCirqAvailability:
    ...
    def test_is_cirq_available_returns_bool(self) -> None:
...
class TestCirqRunnerWithoutCirq:
    ...
    def test_runner_raises_import_error(self) -> None:
...
class TestCirqRunnerInit:
    ...
    def test_qubit_count_matches_K(self) -> None:
        for n in (2, 3, 4):
            K = np.eye(n) * 0.3
            omega = np.ones(n)
            runner = CirqRunner(K, omega)
...
class TestCirqTrotterCircuit:
    ...
    def test_trotter_step_returns_circuit(self) -> None:
        import cirq
    ...
    def test_zero_coupling_no_xx_yy_gates(self) -> None:
    ...
    def test_zero_omega_no_z_gates(self) -> None:
...
class TestCirqRunTrotter:
    ...
    def test_run_trotter_returns_cirq_result(self) -> None:
    ...
    def test_energy_is_real_finite(self) -> None:
        K = np.array([[0, 0.5], [0.5, 0]])
    ...
    def test_run_trotter_various_sizes(self, n: int) -> None:
...
class TestCirqComputeEnergy:
    ...
    def test_all_zero_state_energy(self) -> None:
    ...
    def test_energy_normalised_state(self) -> None:
        """Uniform superposition should give zero Z expectation."""
        K = np.zeros((2, 2))
        omega = np.array([1.0, 1.0])
        runner = CirqRunner(K, omega)
        sv = np.ones(4, dtype=np.complex128) / 2.0  # |++>
        e = runner._compute_energy(sv)
