...
"""Lock the BL-19 whole-program AD product gate into preflight and CI."""
...
def test_static_gate_is_strict_and_numpy_documented() -> None:
    ...
    assert "D,D413" in gates["ruff D whole-program-ad-product quality ratchet"]


def test_coverage_gate_is_isolated_and_exact() -> None:
    """Require branch execution and exact source-only coverage."""
    gates = dict(quality_gates.build_coverage_gates("/python"))
    assert "--branch" in gates["whole-program-ad-product focused coverage"]
    assert "--fail-under=100" in gates["whole-program-ad-product exact coverage threshold"]
