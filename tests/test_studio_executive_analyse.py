...
if TYPE_CHECKING:
    ...
    from scpn_quantum_control.studio.executive_analyse import (
        ANALYSE_VERB,
...
def test_analyse_synchronised_cloud_witnesses_one_component() -> None:
    """Measure a synchronised cloud through the real witness implementation."""
...
def test_analyse_two_cluster_cloud_counts_two_components() -> None:
    """Resolve two persistent components for a separated phase cloud."""
...
def test_analyse_loop_cloud_has_dominant_h1_lifetime() -> None:
    """Expose a non-zero dominant loop lifetime for a circular phase cloud."""
...
def test_analyse_plan_defaults_backend_read_only() -> None:
    """Expose the read-only plan with its default NumPy backend."""
...
def test_analyse_accepts_declared_rust_backend() -> None:
    """Accept the Rust backend declared by the analyse contract."""
...
def test_analyse_rejects_undeclared_backend() -> None:
    """Reject a backend absent from the public analyse contract."""
...
def test_generated_analyse_script_embeds_summary_and_compiles() -> None:
    """Generate valid source carrying the sealed witness summary."""
    record = run_action(_request(), registry=_registry())
    assert record.script is not None
...
def test_generated_analyse_script_executes_real_witness(tmp_path: Path) -> None:
    """Run the generated script against the real synchronisation witness."""
    record = run_action(_request(), registry=_registry())
    assert record.script is not None
    ...
    assert "order_parameter=" in completed.stdout
    assert completed.stdout.rstrip().endswith("components=1 verified")
...
def test_normalise_analyse_rejects_invalid(overrides: dict[str, Any]) -> None:
    """Reject malformed or unbounded phase-cloud requests."""
    parameters = dict(_CLOUD)
    parameters.update(overrides)
    with pytest.raises(ValueError):
...
def test_normalise_analyse_accepts_bounded_cloud() -> None:
    """Normalise a bounded phase cloud and its witness parameters."""
