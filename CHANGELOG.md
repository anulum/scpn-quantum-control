...
- 2026-07-24 — PennyLane + Qiskit migration guides product:
  ...
  parity. Docs: `docs/migration_guides_product.md`. The bounded adoption facade
  delegates execution to the existing local PennyLane and Qiskit integrations.
...
- 2026-07-17 — Matched-basis QBER re-run EXECUTED under owner GO:
  ...
  (1.95–3.70%) and far below the published 5.5%/5.8%, whose
...
- 2026-07-14 — Restored lock-faithful wheel builds across the Python 3.11,
  ...
  hashed locks are regenerated by their matching interpreters, and a permanent
  audit cross-checks both projects, direct pins, source-verified hashes,
  pip-compile ownership, installed backend, CI/Docker consumers, and the real
  no-isolation wheel fixture. The security job now exposes both source trees so
  its focused pytest coverage gate can import the shared test configuration.
  CI enforces strict typing, NumPy docstrings, Bandit, and exact
...
- 2026-07-13 — Closed the D401 suppression and adjacent module-docstring D413
  ...
  ground-state Hamiltonian helper. Three strictly typed branch regressions
  cover the optional-native-symbol fallback, deterministic noise-free
  measurement, and zero-curvature peak refinement; scientific algorithms,
  the curated top-level API, native-kernel behavior, and benchmark evidence
  are unchanged.
