...
raw-count pack) and ``studio.qpu-result-pack.v1`` (the unit that binds the
raw-results digest, calibration snapshot, and bit-exact circuit digest to a
provider attestation). An attached attestation stays ``present_unverified``
until an enrolled provider-key verification path exists. Both use
``verifiability_mode = attestation``: a QPU result cannot be recomputed in a
verifier's browser."""
