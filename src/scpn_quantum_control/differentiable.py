...
def _program_ad_interp_static_boundary(name: str, value: object) -> float | None:
    ...
    if isinstance(value, (TraceADArray, TraceADScalar)):
        raise ValueError(f"program AD np.interp {name} boundary must be static real numeric")
    boundary = _as_real_scalar(f"program AD interpolation interp {name}", value)
    if not math.isfinite(boundary):
        raise ValueError(f"program AD interpolation interp {name} must be finite")
    return boundary


def _program_ad_interp_source_size(sample_shape: tuple[int, ...], grid_size: int) -> int:
    return _program_ad_array_static_size(sample_shape) + grid_size
...
def _program_ad_signal_derivative_rule(name: str) -> CustomDerivativeRule:
    if name == "convolve":
        return CustomDerivativeRule(
            name="program_ad_signal_convolve_trace_contract",
...
def _program_ad_signal_lowering_metadata(name: str) -> Mapping[str, str]:
    ...
    return {
        ...
        "mlir_op": "scpn_diff.signal.convolve",
        "llvm": "blocked_until_executable_signal_lowering",
        "rust": "blocked_until_polyglot_signal_ad",
        "static_argument_rule": "required",
        "static_derivative_factory": "program_ad_signal_convolve_derivative_rule",
    ...
    for name, identity in _PROGRAM_AD_SIGNAL_IDENTITIES.items():
        ...
        DEFAULT_CUSTOM_DERIVATIVE_REGISTRY.register_transform(
            PrimitiveTransformRule(
                ...
                shape_rule=_program_ad_signal_convolve_shape,
                dtype_rule=_program_ad_signal_convolve_dtype_rule,
                static_argument_rule=_program_ad_signal_convolve_static_arguments,
