...
class QPUDataArtifact:
    ...
    ----------
    domain
        Application domain that produced the oscillator data.
    source_name
        Human-readable source dataset or stream name.
    source_mode
        Provenance mode from ``ALL_SOURCE_MODES``.
    K_nm
        Finite symmetric non-negative coupling matrix with zero diagonal.
    omega
        Finite natural-frequency vector aligned with ``K_nm``.
    theta0
        Optional finite initial-phase vector aligned with ``omega``.
    layer_assignments
        Optional oscillator-to-layer labels.
    normalization
        Description of the applied normalization.
    extraction_method
        Description of the source-to-oscillator extraction method.
    source_timestamp
        Optional source acquisition timestamp.
    replay_id
        Optional reproducible source-run identifier.
    metadata
        Recursively immutable JSON-compatible provenance metadata.
    hashes
        SHA-256 digests of the canonical numeric arrays.
    ...
    def __post_init__(self) -> None:
        """Validate, hash, copy, and freeze all artifact fields."""
    ...
    def require_publication_safe(self) -> None:
        ...
        ------
        ValueError
            If the source mode is synthetic or lacks a timestamp and replay ID.
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialise to a JSON-compatible hash-locked mapping.

        Returns
        -------
        dict[str, Any]
            Canonical artifact payload with an ``artifact_sha256`` digest.
    ...
    def to_json(self, *, indent: int | None = 2) -> str:
        """Serialise to deterministic key-sorted JSON.

        Parameters
        ----------
        indent
            JSON indentation width, or ``None`` for compact output.

        Returns
        -------
        str
            Serialized hash-locked artifact.
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> QPUDataArtifact:
        ...
        ----------
        data
            Versioned artifact mapping, optionally carrying its artifact hash.
        ...
        ------
        ValueError
            If the schema, fields, array hashes, or artifact hash are invalid.
    ...
    def from_json(cls, payload: str) -> QPUDataArtifact:
        ...
        ----------
        payload
            JSON-encoded versioned artifact mapping.
    ...
    def from_scpn_datastream_payload(
        ...
        *,
        ...
        ----------
        payload
            Canonical ``sc-neurocore.scpn.datastream.v1`` payload.
        source_mode
            Smoke-test provenance mode for the adapted artifact.
        domain
            Domain label assigned to the artifact.
        source_name
            Source stream name assigned to the artifact.
        normalization
            Normalization description assigned to the artifact.
        extraction_method
            Extraction-method description assigned to the artifact.

        Returns
        -------
        QPUDataArtifact
            Hash-locked synthetic artifact with deterministic replay identity.

        Raises
        ------
        ValueError
            If the stream schema, replay fields, arrays, or identity fields are
            invalid, or if a non-smoke-test provenance mode is requested.
...
def write_qpu_data_artifact(path: str | Path, artifact: QPUDataArtifact) -> None:
    ...
    artifact
        Validated artifact to serialize with its digest.
...
def validate_qpu_data_artifact(
    ...
    *,
    ...
    ----------
    artifact
        Existing artifact or versioned artifact mapping.
    require_publication_safe
        Whether to reject synthetic or insufficiently traceable provenance.
    ...
    ------
    ValueError
        If artifact validation or the requested publication-safety gate fails.
...
def artifact_from_arrays(
    *,
    ...
    """Construct a QPU data artifact from oscillator arrays.
    ...
    normalization
        Applied normalization description.
    extraction_method
        Source-to-oscillator extraction description.
    theta0
        Optional initial-phase vector.
    layer_assignments
        Optional oscillator-to-layer labels.
    source_timestamp
        Optional source acquisition timestamp.
    replay_id
        Optional reproducible source-run identifier.
    metadata
        Optional JSON-compatible provenance metadata.

    Returns
    -------
    QPUDataArtifact
        Validated immutable and hash-locked artifact.
