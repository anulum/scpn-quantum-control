...
# SCPN Quantum Control — Neural-operator surrogate for Kuramoto network dynamics
...
This module is an *optional* capability — it requires PyTorch (``scpn-quantum-control[torch]``) and so
...
def _require_torch() -> Any:
    try:
        ...
        raise ImportError(
            "the Kuramoto neural operator requires PyTorch; install scpn-quantum-control[torch]"
        ) from error
    return torch
