...
Rust-accelerated pipeline ($5{,}401\times$
...
diagnostics for systems of 2--16~qubits. Legacy ibm\_fez artifacts include
Bell, QKD, VQE, ZNE, Trotter, and UPDE rows that must be cited by committed
artifact path rather than as broad validation. Finite-size scaling yields
...
level statistics at $n = 8$) without deep many-body localisation. A
hardware-anchored scaling analysis places the exact Hilbert-space
simulation crossover at $n \approx 11.6$, while leaving broad quantum
advantage open because Rust Kuramoto ODE baselines remain faster for the
measured $n \leq 16$ regime.
...
The March~2026 \textbf{ibm\_fez} material is retained as legacy
artifact-backed evidence. Individual Bell, QKD, VQE, ZNE, Trotter, and UPDE
values must cite their committed result artifact or the hardware ledger. The
...
Simulation data, ledgered hardware artifacts, Python modules, Rust engine
