...
\newcommand{\paperdoi}{Zenodo DOI: \href{https://doi.org/10.5281/zenodo.20382333}{10.5281/zenodo.20382333}}
...
A typical user is a graduate student or hardware-methods researcher who wants
to reproduce a small Kuramoto--XY benchmark without rebuilding the full
artefact chain by hand. From a clean checkout, that user can install the
package, regenerate the $n=4$ VQE comparison, inspect the submitted IBM job
metadata, and rerun the count reducer that produced the manuscript table. This
use case is intentionally narrower than general-purpose SDK development: the
package is designed to make one class of oscillator-network hardware study
...
The $44.1\times$ local $K_{ij}$ row is a kernel result, not an end-to-end VQE
speedup. It matters because coupling construction is a repeated inner-loop
operation in topology sweeps, ansatz generation, Hamiltonian export, and
benchmark regeneration. The manuscript therefore reports it as a bounded
software-engineering gain under an Amdahl-law caveat: once Qiskit
transpilation, provider queueing, optimiser evaluations, and raw-count
analysis dominate, kernel acceleration improves workflow latency and
reproducibility rather than becoming a full-stack performance claim.
