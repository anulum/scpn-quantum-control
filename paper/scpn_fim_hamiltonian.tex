...
$\lambda\in\{0,1,4\}$. Finally, we executed the corresponding n=4 IBM Heron r2
hardware tests on \texttt{ibm\_kingston}. A single-sample pilot was followed by
a repeated randomized run with 166 circuits and 339968 shots. The repeated run
falsifies the simple hardware-protection interpretation for this
implementation: at $\lambda=4$ relative to $\lambda=0$, magnetisation leakage
increased by $0.0896$ on average across matched state/depth comparisons
(Fisher combined $p=7.49\times10^{-55}$), while exact-state retention
decreased by $0.0797$ on average. The paper is deliberately bounded: the
artefacts support exact small-system Hamiltonian findings and a negative
hardware result for this backend/circuit family. They do not demonstrate
...
small-system energy landscape, sector structure, spectral diagnostics, low-energy
entanglement, and hardware-test protocol for Kuramoto--XY circuits?
...
improves hardware coherence on \texttt{ibm\_kingston}.
...
All numerical statements in this draft are generated from committed artefacts
under \texttt{data/scpn\_fim\_hamiltonian/}. The scripts are:
%
\begin{itemize}
  \item \texttt{scripts/analyse\_fim\_spectrum.py},
  \item \texttt{scripts/analyse\_fim\_level\_spacing.py},
  \item \texttt{scripts/analyse\_fim\_entanglement.py},
  \item \texttt{scripts/analyse\_fim\_sector\_survival.py},
  \item \texttt{scripts/benchmark\_fim\_vqe\_ground\_state.py}, and
  \item \texttt{scripts/analyse\_fim\_ibm\_repeated\_followup.py}.
\end{itemize}
%
...
The IBM hardware component was designed as a falsification test. Because
...
$\lambda=0$ for the tested implementation.
...
native analog implementations, different layouts, lower-depth constructions, or
future devices. It does show that the direct all-pair digital implementation
tested here is not a hardware-protection mechanism on this backend and
calibration window.
...
full confusion-matrix inversion. No hardware coherence improvement, quantum
advantage, or universal protection claim is made.

\section{Data and code availability}

Code, scripts, and artefacts are maintained in the public repository
\href{https://github.com/anulum/scpn-quantum-control}{GitHub repository}.
...
Generative AI assistance was used during software drafting, artefact-harness
construction, and manuscript editing. Numerical claims in this paper were not
accepted from generated text; they were checked against committed JSON/CSV
artefacts and corresponding scripts. Human review controls scientific framing,
claim boundaries, authorship decisions, and final responsibility for any
