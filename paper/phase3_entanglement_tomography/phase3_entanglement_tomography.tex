...
while the FIM control improves. The result is a mechanism-boundary measurement:
...
\texttt{d86ijr9789is738vnh30}. A same-layout statistical repeat was submitted
after this analysis as a drift-robustness check; it is not used in the present
tables because it was still pending at manuscript update time.
...
same-layout Kingston repeat submitted after this reduction is reserved for a
statistical drift check; the manuscript should use it only after both jobs are
completed and reduced under the same preregistered criterion.
...
\begin{itemize}
  ...
  \item pending Kingston same-layout statistical repeat artefact:
...
same-layout statistical repeat has been queued as a drift-robustness check.
...
small controlled noise-scaling stress test. It still avoids scalable
