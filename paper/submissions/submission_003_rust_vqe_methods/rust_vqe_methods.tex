...
workload. A same-day 25-circuit IBM Marrakesh validation lane preserves
the same ansatz energy ordering after full 16-state readout calibration:
$K_{ij}$-informed $-6.2021$, \texttt{TwoLocal} $-6.0805$, and
\texttt{EfficientSU2} $-6.0539$. The paper is
...
ansatz-selection rule. The IBM observable comparison in
Table~\ref{tab:hardwareansatz} is a single-backend, single-layout
validation lane; it should be read as a hardware sanity check for the
methods claim, not as backend-general ansatz superiority.
