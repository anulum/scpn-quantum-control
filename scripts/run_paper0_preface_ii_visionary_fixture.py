...
# scpn-quantum-control -- Paper 0 Preface II visionary runner
"""Run the Paper 0 Preface II visionary validation fixture."""
...
def render_report(payload: dict[str, Any]) -> str:
    """Render a compact Markdown report for the Preface II visionary fixture."""
    return "\n".join(
        [
            "# Paper 0 Preface II Visionary Fixture",
            "",
            ...
            f"- Status and Method boundary: {payload['status_method_boundary']}",
            "",
            "## Visionary Operators",
            *(
                f"- `{key}`: {value['active_inference_role']} / {value['sigma_role']}"
                for key, value in sorted(payload["visionary_operators"].items())
            ),
            "",
            "## Null Controls",
...
def main() -> int:
    """Run the Preface II visionary fixture and write artefacts."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "paper0_preface_ii_visionary_fixture_result_2026-05-13.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "paper0_preface_ii_visionary_fixture_report_2026-05-13.md",
    )
