...
# scpn-quantum-control -- Paper 0 Status and Method continuation runner
"""Run the Paper 0 Status and Method continuation validation fixture."""
...
def render_report(payload: dict[str, Any]) -> str:
    """Render a compact Markdown report for the continuation fixture."""
    return "\n".join(
        [
            "# Paper 0 Status and Method Continuation Fixture",
            "",
            f"- Source span: {payload['source_ledger_span'][0]} - "
            f"{payload['source_ledger_span'][1]}",
            f"- Hardware status: {payload['hardware_status']}",
            f"- Claim boundary: {payload['claim_boundary']}",
            f"- Operational commitments: {payload['operational_commitment_count']}",
            f"- Blank separators: {payload['blank_separator_count']}",
            f"- SCPN mandate boundary: {payload['scp_mandate_boundary']}",
            "",
            "## Disagreement Moves",
            *(
                f"- `{key}`: {value}"
                for key, value in sorted(payload["disagreement_moves"].items())
            ),
            "",
            "## Null Controls",
...
def main() -> int:
    """Run the Status and Method continuation fixture and write artefacts."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_DIR
        / "paper0_status_method_continuation_fixture_result_2026-05-13.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=DEFAULT_OUTPUT_DIR
        / "paper0_status_method_continuation_fixture_report_2026-05-13.md",
    )
