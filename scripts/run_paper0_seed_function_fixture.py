...
# scpn-quantum-control -- Paper 0 seed-function fixture runner
"""Run Paper 0 teleological seed-function simulator fixtures."""
...
def run_default_fixture() -> SeedFunctionFixtureResult:
    """Run default simulator-only seed-function fixture."""
    return validate_seed_function_fixture()
...
def render_report(result: SeedFunctionFixtureResult, runtime_ms: float) -> str:
    """Render a Markdown report for the seed-function fixture."""
    return "\n".join(
        [
            "# Paper 0 Seed Function Fixture",
            "",
            f"- Hardware status: `{result.hardware_status}`",
            f"- Source span: `{', '.join(result.source_ledger_span)}`",
            f"- SSB bias magnitude: `{result.payload['ssb_bias_magnitude']:.6f}`",
            f"- Is random reset: `{result.payload['is_random_reset']}`",
            f"- Conformal continuity: `{result.payload['conformal_continuity']}`",
            f"- Runtime ms: `{runtime_ms:.6f}`",
            f"- Claim boundary: `{result.claim_boundary}`",
            "",
            "Passing this fixture is not empirical evidence and does not validate "
...
def write_outputs(
    result: SeedFunctionFixtureResult,
    *,
    ...
    json_path = output_dir / f"paper0_seed_function_fixture_result_{date_tag}.json"
    report_path = output_dir / f"paper0_seed_function_fixture_report_{date_tag}.md"
