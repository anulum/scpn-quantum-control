...
# SCPN Quantum Control — Multi-tier benchmark for the Sakaguchi–Kuramoto mean-field force and Jacobian
...
hand-transcription, mirroring ``bench_mean_field_tiers.py``.

This script is the authoritative source for the Sakaguchi mean-field chain ordering recorded in
``src/scpn_quantum_control/accel/sakaguchi_mean_field.py``. A tier that is not installed is
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark Sakaguchi mean-field force/Jacobian (alpha=0.5) across tiers"
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/sakaguchi_mean_field_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        ...
        return False
    try:
        sakaguchi_mean_field_force(np.zeros(8, dtype=np.float64), 1.0, 0.5)
        sakaguchi_mean_field_jacobian(np.zeros(8, dtype=np.float64), 1.0, 0.5)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    """Run the Sakaguchi mean-field tier benchmark CLI for the force and the Jacobian."""
    ...
    from scpn_quantum_control.accel import sakaguchi_mean_field as smf

    rust_ok = disp._rust_available()
    julia_ok = disp._julia_available()
    force_tiers: dict[str, _Tier | None] = {
        "rust": partial(smf._rust_sakaguchi_mean_field_force, coupling=1.0, frustration=0.5)
        if rust_ok
        else None,
        "julia": partial(smf._julia_sakaguchi_mean_field_force, coupling=1.0, frustration=0.5)
        if julia_ok
        else None,
        "python": partial(smf._python_sakaguchi_mean_field_force, coupling=1.0, frustration=0.5),
    }
    jacobian_tiers: dict[str, _Tier | None] = {
        "rust": partial(smf._rust_sakaguchi_mean_field_jacobian, coupling=1.0, frustration=0.5)
        if rust_ok
        else None,
        "julia": partial(smf._julia_sakaguchi_mean_field_jacobian, coupling=1.0, frustration=0.5)
        if julia_ok
        else None,
        "python": partial(
            smf._python_sakaguchi_mean_field_jacobian, coupling=1.0, frustration=0.5
        ),
    }
    ...
    payload = {
        ...
        "sakaguchi_mean_field_force": _bench_function(
            "sakaguchi_mean_field_force (K=1, alpha=0.5)", force_tiers, sizes, args, rng
        ),
        "sakaguchi_mean_field_jacobian": _bench_function(
            "sakaguchi_mean_field_jacobian (K=1, alpha=0.5)", jacobian_tiers, sizes, args, rng
        ),
    }
