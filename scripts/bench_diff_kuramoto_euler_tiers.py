...
# scpn-quantum-control — Differentiable Kuramoto Euler tier benchmark
"""Multi-tier benchmark for the differentiable networked-Kuramoto Euler integrator.

Times the forward trajectory ``kuramoto_euler_trajectory`` and its reverse-mode adjoint
``kuramoto_euler_vjp`` (Rust → Julia → Python floor) on a dense random coupling matrix over a
...
comments in ``src/scpn_quantum_control/accel/diff_kuramoto_euler.py``.
"""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/diff_kuramoto_euler_tiers.json"),
...
def main(argv: list[str] | None = None) -> int:
    """Run the differentiable-Kuramoto-Euler tier benchmark for the forward and the adjoint."""
    args = _parse_args(argv if argv is not None else sys.argv[1:])
    sizes = [int(token) for token in args.sizes.split(",") if token.strip()]
    n_steps = args.n_steps

    from scpn_quantum_control.accel import diff_kuramoto_euler as dk
    from scpn_quantum_control.accel import dispatcher as disp

    rust_ok = disp.optional_rust_engine() is not None
    julia_ok = False
    try:
        from scpn_quantum_control.accel.julia import kuramoto_euler_trajectory as _jt
        from scpn_quantum_control.accel.julia import kuramoto_euler_vjp as _jv

        _theta = np.zeros(8, dtype=np.float64)
        _k = np.zeros((8, 8), dtype=np.float64)
        _traj = _jt(_theta, _theta, _k, 0.05, 4)
        _jv(_traj, _k, 0.05, _theta)
        julia_ok = True
    ...
    def vjp_builder(
        ...
        def build(size: int, rng: np.random.Generator) -> _Thunk:
            theta0, omega, coupling = _problem(size, rng)
            trajectory = dk._python_kuramoto_euler_trajectory(
                theta0, omega, coupling, 0.05, n_steps
            )
            cotangent = np.cos(trajectory[-1])
            return lambda: impl(trajectory, coupling, 0.05, cotangent)

        return build

    forward_tiers: dict[str, Callable[[int, np.random.Generator], _Thunk | None]] = {
        "rust": forward_builder(dk._rust_kuramoto_euler_trajectory) if rust_ok else _none,
        "julia": forward_builder(dk._julia_kuramoto_euler_trajectory) if julia_ok else _none,
        "python": forward_builder(dk._python_kuramoto_euler_trajectory),
    }
    vjp_tiers: dict[str, Callable[[int, np.random.Generator], _Thunk | None]] = {
        "rust": vjp_builder(dk._rust_kuramoto_euler_vjp) if rust_ok else _none,
        "julia": vjp_builder(dk._julia_kuramoto_euler_vjp) if julia_ok else _none,
        "python": vjp_builder(dk._python_kuramoto_euler_vjp),
    }
    ...
    payload = {
        ...
        "kuramoto_euler_trajectory": _bench(
            f"kuramoto_euler_trajectory (n_steps={n_steps})", forward_tiers, sizes, args, rng
        ),
        "kuramoto_euler_vjp": _bench(
            f"kuramoto_euler_vjp (n_steps={n_steps})", vjp_tiers, sizes, args, rng
        ),
    }
