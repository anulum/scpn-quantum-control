...
# scpn-quantum-control -- Paper 0 Axiom I Psi-field fixture runner
"""Run the Paper 0 Axiom I Psi-field source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_psi_field_fixture_result_2026-05-13.json"
)
DEFAULT_REPORT_PATH = DEFAULT_OUTPUT_DIR / "paper0_axiom_i_psi_field_fixture_report_2026-05-13.md"
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Axiom I Psi-Field Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Axiom I role count: {payload['axiom_i_role_count']}",
        f"- Psi-field claim count: {payload['psi_field_claim_count']}",
    ...
    for key, role in payload["axiom_i_roles"].items():
        lines.append(f"- `{key}`: `{role}`")
    lines.extend(["", "## Psi-Field Claims"])
    for key, claim in payload["psi_field_claims"].items():
        lines.append(f"- `{key}`: `{claim}`")
