...
# scpn-quantum-control — Symplectic inertial (second-order) networked-Kuramoto tier benchmark
"""Multi-tier benchmark for the symplectic inertial networked-Kuramoto forward trajectory.

Times ``networked_symplectic_inertial_trajectory`` across its Rust → Julia → Python floor tier chain
on a dense random coupling matrix over a fixed-step velocity-Verlet horizon, reporting the
median-of-repeats per-call wall-time. The three tiers are tolerance-parity (they share the Verlet
arithmetic and differ only in the coupling-force summation order), so the wall-times are
...
``oscillatools/src/oscillatools/accel/networked_symplectic_inertial.py``.
"""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--dt", type=float, default=0.01, help="Fixed Verlet time step.")
    parser.add_argument("--n-steps", type=int, default=600, help="Number of Verlet steps.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/networked_symplectic_inertial_tiers.json"),
...
def main(argv: list[str] | None = None) -> int:
    """Run the symplectic inertial forward tier benchmark across Rust → Julia → Python."""
    ...
    from oscillatools.accel import networked_symplectic_inertial as ni

    engine = disp.optional_rust_engine()
    rust_ok = engine is not None and hasattr(engine, "kuramoto_symplectic_inertial_trajectory")
    julia_ok = False
    try:
        from oscillatools.accel.julia import kuramoto_symplectic_inertial_trajectory as _ji
    ...
    forward_tiers: dict[str, Callable[[int, np.random.Generator], _Thunk | None]] = {
        "rust": forward_builder(ni._rust_networked_symplectic_inertial_trajectory)
        if rust_ok
        else _none,
        "julia": forward_builder(ni._julia_networked_symplectic_inertial_trajectory)
        if julia_ok
        else _none,
        "python": forward_builder(ni._python_networked_symplectic_inertial_trajectory),
    }
    ...
    payload = {
        ...
        "networked_symplectic_inertial_trajectory": _bench(
            f"networked_symplectic_inertial_trajectory (dt={dt}, n_steps={n_steps})",
            forward_tiers,
            sizes,
            args,
            rng,
