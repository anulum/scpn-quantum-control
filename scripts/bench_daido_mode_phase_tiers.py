...
This script is the authoritative source for the mode-phase chain ordering recorded in
``src/scpn_quantum_control/accel/daido_phase.py``. A tier that is not installed is recorded
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Benchmark daido_mode_phase (m=2) across tiers")
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/daido_mode_phase_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        ...
        return False
    try:
        daido_mode_phase(np.zeros(8, dtype=np.float64), 2)
        daido_mode_phase_gradient(np.zeros(8, dtype=np.float64), 2)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    """Run the mode-phase tier benchmark CLI for the value and the gradient."""
    args = _parse_args(argv if argv is not None else sys.argv[1:])
    sizes = [int(token) for token in args.sizes.split(",") if token.strip()]

    from scpn_quantum_control.accel import daido_phase as dp
    ...
    value_tiers: dict[str, _Tier | None] = {
        "rust": partial(dp._rust_daido_mode_phase, m=2) if rust_ok else None,
        "julia": partial(dp._julia_daido_mode_phase, m=2) if julia_ok else None,
        "python": partial(dp._python_daido_mode_phase, m=2),
    }
    gradient_tiers: dict[str, _Tier | None] = {
        "rust": partial(dp._rust_daido_mode_phase_gradient, m=2) if rust_ok else None,
    ...
    payload = {
        ...
        "daido_mode_phase": _bench_function(
            "daido_mode_phase (m=2)", value_tiers, sizes, args, rng
        ),
