...
stability Jacobian on a dense symmetric random coupling matrix, reporting the
median-of-repeats per-call wall-time. Both routes are O(N^2) (the force sums over the full
coupling row), so the sizes stop at N = 2048. The output is written as JSON so it can be
embedded in ``docs/pipeline_performance.md`` without hand-transcription, mirroring
``bench_networked_kuramoto_tiers.py``.

This script is the authoritative source for the Sakaguchi chain ordering recorded
in ``src/scpn_quantum_control/accel/sakaguchi_kuramoto.py``. A tier that is not installed is
...
_FRUSTRATION = 0.3
_Tier = Callable[[NDArray[np.float64], NDArray[np.float64], float], Any]


def _parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse the benchmark command-line arguments."""
    parser = argparse.ArgumentParser(
        description="Benchmark Kuramoto–Sakaguchi force/Jacobian across tiers"
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/sakaguchi_kuramoto_tiers.json"),
...
def _measure(
    ...
    for _ in range(inner_repeats):
        _ = fn(theta, coupling, _FRUSTRATION)
...
def _warm_julia() -> bool:
    ...
    try:
        ...
        sakaguchi_force(theta, coupling, _FRUSTRATION)
        sakaguchi_jacobian(theta, coupling, _FRUSTRATION)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    """Run the Sakaguchi tier benchmark CLI for the force and the Jacobian."""
    ...
    from scpn_quantum_control.accel import sakaguchi_kuramoto as sk

    rust_ok = disp._rust_available()
    julia_ok = disp._julia_available()
    force_tiers: dict[str, _Tier | None] = {
        "rust": sk._rust_sakaguchi_force if rust_ok else None,
        "julia": sk._julia_sakaguchi_force if julia_ok else None,
        "python": sk._python_sakaguchi_force,
    }
    jacobian_tiers: dict[str, _Tier | None] = {
        "rust": sk._rust_sakaguchi_jacobian if rust_ok else None,
        "julia": sk._julia_sakaguchi_jacobian if julia_ok else None,
        "python": sk._python_sakaguchi_jacobian,
    }
    ...
    payload = {
        ...
        "sakaguchi_force": _bench_function(
            "sakaguchi_force (alpha=0.3)", force_tiers, sizes, args, rng
        ),
        "sakaguchi_jacobian": _bench_function(
            "sakaguchi_jacobian (alpha=0.3)", jacobian_tiers, sizes, args, rng
        ),
    }
