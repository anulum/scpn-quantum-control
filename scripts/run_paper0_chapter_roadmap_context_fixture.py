...
# scpn-quantum-control -- Paper 0 chapter roadmap context runner
"""Run the Paper 0 chapter roadmap context validation fixture."""
...
def render_report(payload: dict[str, Any]) -> str:
    """Render a compact Markdown report for the chapter roadmap fixture."""
    return "\n".join(
        [
            "# Paper 0 Chapter Roadmap Context Fixture",
            "",
            f"- Source span: {payload['source_ledger_span'][0]} - "
            f"{payload['source_ledger_span'][1]}",
            f"- Hardware status: {payload['hardware_status']}",
            f"- Claim boundary: {payload['claim_boundary']}",
            f"- Parts: {payload['part_count']}",
            f"- Chapters: {payload['chapter_count']}",
            f"- Blank markers: {payload['blank_marker_count']}",
            f"- Numbering inconsistency present: {payload['numbering_inconsistency_present']}",
...
def main() -> int:
    """Run the chapter roadmap context fixture and write artefacts."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_DIR
        / "paper0_chapter_roadmap_context_fixture_result_2026-05-13.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "paper0_chapter_roadmap_context_fixture_report_2026-05-13.md",
    )
