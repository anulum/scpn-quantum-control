...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark order_parameter_hessian across tiers",
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/order_parameter_hessian_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        from scpn_quantum_control.accel.julia import order_parameter_hessian as julia_grad
    except Exception:
        return False
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    tiers: dict[str, _HessianTier | None] = {
        "rust": d._rust_order_parameter_hessian if d._rust_available() else None,
        "julia": d._julia_order_parameter_hessian if d._julia_available() else None,
