...
# scpn-quantum-control -- Paper 0 Axiom I meta-coupling fixture runner
"""Run the Paper 0 Axiom I meta-coupling source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_meta_coupling_fixture_result_2026-05-13.json"
)
DEFAULT_REPORT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_meta_coupling_fixture_report_2026-05-13.md"
)
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Axiom I Meta-Coupling Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Predictive hardware role count: {payload['predictive_hardware_role_count']}",
        f"- Interaction component count: {payload['interaction_component_count']}",
        f"- Next source boundary: {payload['next_source_boundary']}",
        f"- Protocol state: {payload['problem_metadata']['protocol_state']}",
        "",
        "## Predictive Hardware Roles",
    ]
    for key, role in payload["predictive_hardware_roles"].items():
        lines.append(f"- `{key}`: `{role}`")
    lines.extend(["", "## Coupling Requirements"])
    for key, requirement in payload["coupling_requirements"].items():
