...
# scpn-quantum-control -- Paper 0 terminology bridge fixture runner
"""Run the Paper 0 terminology-bridge source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_terminology_bridge_fixture_result_2026-05-13.json"
)
DEFAULT_REPORT_PATH = DEFAULT_OUTPUT_DIR / "paper0_terminology_bridge_fixture_report_2026-05-13.md"
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Terminology Bridge Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Mainstream anchor count: {payload['mainstream_anchor_count']}",
        f"- Analogy boundary count: {payload['analogy_boundary_count']}",
        f"- Next source boundary: {payload['next_source_boundary']}",
        "",
        "## Mainstream Anchors",
    ]
    for key, anchor in payload["mainstream_anchors"].items():
        lines.append(f"- `{key}`: `{anchor}`")
    lines.extend(["", "## PELA Boundaries"])
    for key, boundary in payload["pela_boundaries"].items():
        lines.append(f"- `{key}`: `{boundary}`")
