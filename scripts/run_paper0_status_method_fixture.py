...
# scpn-quantum-control -- Paper 0 Status and Method runner
"""Run the Paper 0 Status and Method validation fixture."""
...
def render_report(payload: dict[str, Any]) -> str:
    """Render a compact Markdown report for the Status and Method fixture."""
    return "\n".join(
        [
            "# Paper 0 Status and Method Fixture",
            "",
            ...
            "",
            "## Scientific Inference Roles",
            *(
                f"- `{key}`: {value}"
                for key, value in sorted(payload["scientific_inference_roles"].items())
            ),
            "",
            "## Null Controls",
...
def main() -> int:
    """Run the Status and Method fixture and write artefacts."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "paper0_status_method_fixture_result_2026-05-13.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "paper0_status_method_fixture_report_2026-05-13.md",
    )
