...
# scpn-quantum-control -- Paper 0 advanced mechanisms fixture runner
"""Run Paper 0 advanced mechanisms simulator fixtures."""
...
def run_default_fixture() -> AdvancedMechanismsFixtureResult:
    """Run default simulator-only advanced mechanisms fixture."""
    return validate_advanced_mechanisms_fixture()
...
def render_report(result: AdvancedMechanismsFixtureResult, runtime_ms: float) -> str:
    """Render a Markdown report for the advanced mechanisms fixture."""
    return "\n".join(
        [
            "# Paper 0 Advanced Mechanisms Fixture",
            "",
            f"- Hardware status: `{result.hardware_status}`",
            f"- Source span: `{', '.join(result.source_ledger_span)}`",
            f"- Gauge transduction: `{result.gauge_transduction:.6f}`",
            f"- Holographic encoding: `{result.holographic_encoding:.6f}`",
            f"- Holographic retrieval: `{result.holographic_retrieval:.6f}`",
            f"- Consilium Pareto support: `{result.consilium_pareto_support:.6f}`",
            f"- Runtime ms: `{runtime_ms:.6f}`",
            f"- Claim boundary: `{result.claim_boundary}`",
            "",
            "Passing this fixture is not empirical evidence and does not validate gauge "
...
def write_outputs(
    result: AdvancedMechanismsFixtureResult,
    *,
    ...
    json_path = output_dir / f"paper0_advanced_mechanisms_fixture_result_{date_tag}.json"
    report_path = output_dir / f"paper0_advanced_mechanisms_fixture_report_{date_tag}.md"
