...
# scpn-quantum-control -- Paper 0 axiomatic Ntilde fixture runner
"""Run the Paper 0 formal Logos/Ntilde source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = DEFAULT_OUTPUT_DIR / "paper0_axiomatic_ntilde_fixture_result_2026-05-13.json"
DEFAULT_REPORT_PATH = DEFAULT_OUTPUT_DIR / "paper0_axiomatic_ntilde_fixture_report_2026-05-13.md"
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Axiomatic Ntilde Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Axiom count: {payload['axiom_count']}",
        f"- Ntilde formula count: {payload['ntilde_formula_count']}",
        f"- Reference regime: {payload['reference_regime']}",
        f"- Reference delta_irr: {payload['reference_delta_irr']}",
        f"- Next source boundary: {payload['next_source_boundary']}",
        "",
        "## Axiomatic Statuses",
    ]
    for key, status in payload["axiomatic_statuses"].items():
