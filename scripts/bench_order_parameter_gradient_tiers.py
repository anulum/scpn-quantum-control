...
and reports the median-of-repeats per-call wall-time. The output is written as
JSON so it can be embedded in ``docs/pipeline_performance.md`` without
hand-transcription, mirroring ``bench_order_parameter_tiers.py``.

This script is the authoritative source for the gradient-chain ordering recorded
in ``src/scpn_quantum_control/accel/dispatcher.py``. A tier that is not installed
is recorded as ``unavailable`` rather than fabricated.
...
.. code-block:: shell
    ...
    python scripts/bench_order_parameter_gradient_tiers.py --sizes 4,16,64,256
    python scripts/bench_order_parameter_gradient_tiers.py --repeats 11
"""
...
_GradientTier = Callable[[NDArray[np.float64]], NDArray[np.float64]]


def _parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse the benchmark command-line arguments."""
    parser = argparse.ArgumentParser(
        description="Benchmark order_parameter_gradient across tiers",
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/order_parameter_gradient_tiers.json"),
        help="Where to write the JSON result.",
    )
    return parser.parse_args(argv)


def _measure(fn: _GradientTier, theta: NDArray[np.float64], inner_repeats: int) -> float:
...
def main(argv: list[str] | None = None) -> int:
    """Run the order-parameter-gradient tier benchmark CLI."""
    ...
    tiers: dict[str, _GradientTier | None] = {
        "rust": d._rust_order_parameter_gradient if d._rust_available() else None,
        "julia": d._julia_order_parameter_gradient if d._julia_available() else None,
