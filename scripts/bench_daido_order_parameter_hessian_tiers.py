...
This script is the authoritative source for the Daido Hessian-chain ordering recorded in
...
.. code-block:: shell
    ...
    python scripts/bench_order_parameter_hessian_tiers.py --sizes 4,16,64,256
    python scripts/bench_order_parameter_hessian_tiers.py --repeats 11
"""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark daido_order_parameter_hessian (m=2) across tiers",
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/daido_order_parameter_hessian_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        from scpn_quantum_control.accel.julia import daido_order_parameter_hessian as julia_grad
    except Exception:
        return False
    try:
        julia_grad(np.zeros(8, dtype=np.float64), 2)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    from scpn_quantum_control.accel import daido_observables as d
    from scpn_quantum_control.accel import dispatcher as _disp

    tiers: dict[str, _HessianTier | None] = {
        "rust": partial(d._rust_daido_order_parameter_hessian, m=2)
        if _disp._rust_available()
        else None,
        "julia": partial(d._julia_daido_order_parameter_hessian, m=2)
        if _disp._julia_available()
        else None,
        "python": partial(d._python_daido_order_parameter_hessian, m=2),
    }
