...
# scpn-quantum-control -- Paper 0 Anulum Collection mandate runner
"""Run the Paper 0 Anulum Collection mandate validation fixture."""
...
def render_report(payload: dict[str, Any]) -> str:
    """Render a compact Markdown report for the mandate fixture."""
    return "\n".join(
        [
            "# Paper 0 Anulum Collection Mandate Fixture",
            "",
            f"- Source span: {payload['source_ledger_span'][0]} - "
            f"{payload['source_ledger_span'][1]}",
            f"- Hardware status: {payload['hardware_status']}",
            f"- Claim boundary: {payload['claim_boundary']}",
            f"- Books: {payload['book_count']}",
            f"- Meta-frameworks: {payload['meta_framework_count']}",
            ...
            *(
                f"- `{key}`: {value}"
                for key, value in sorted(payload["meta_framework_integrations"].items())
            ),
            "",
            "## Null Controls",
...
def main() -> int:
    """Run the Anulum Collection mandate fixture and write artefacts."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_DIR
        / "paper0_anulum_collection_mandate_fixture_result_2026-05-13.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=DEFAULT_OUTPUT_DIR
        / "paper0_anulum_collection_mandate_fixture_report_2026-05-13.md",
    )
