...
Both routes are O(N^2) (the energy sums over the full coupling matrix), so the sizes stop at
...
This script is the authoritative source for the interaction-energy chain ordering recorded
in ``src/scpn_quantum_control/accel/kuramoto_energy.py``. A tier that is not installed is
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark Kuramoto interaction energy/gradient across tiers"
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/kuramoto_energy_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        ...
        kuramoto_interaction_energy_gradient(theta, coupling)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    """Run the interaction-energy tier benchmark CLI for the energy and the gradient."""
    ...
    from scpn_quantum_control.accel import kuramoto_energy as ke

    rust_ok = disp._rust_available()
    julia_ok = disp._julia_available()
    energy_tiers: dict[str, _Tier | None] = {
        "rust": ke._rust_kuramoto_interaction_energy if rust_ok else None,
        "julia": ke._julia_kuramoto_interaction_energy if julia_ok else None,
        "python": ke._python_kuramoto_interaction_energy,
    }
    gradient_tiers: dict[str, _Tier | None] = {
        "rust": ke._rust_kuramoto_interaction_energy_gradient if rust_ok else None,
    ...
    payload = {
        ...
        "kuramoto_interaction_energy": _bench_function(
            "kuramoto_interaction_energy", energy_tiers, sizes, args, rng
        ),
