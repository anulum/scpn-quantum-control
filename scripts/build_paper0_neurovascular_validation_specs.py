...
# scpn-quantum-control -- Paper 0 neurovascular validation spec builder
"""Promote Paper 0 neurovascular anchors into validation specs."""
...
SPEC_METADATA: dict[str, dict[str, tuple[str, ...] | str]] = {
    "embodied.neurovascular_phase_coupling": {
        "validation_protocol": "paper0.embodied.neurovascular.phase_locking",
        ...
        "null_controls": (
            "set zero K_NH and require neural phase to remain at intrinsic frequency",
            "detune Mayer-wave hemodynamic frequency and require reduced phase locking",
            "shuffle hemodynamic phase drive while preserving marginal phase increments",
            "simulate impaired CBF supply boundary separately from phase entrainment",
        ),
        "executable_validation_targets": (
            "integrate the two-oscillator neural-hemodynamic phase fixture",
            "measure phase locking value and mean frequency slip under finite K_NH",
            "scan K_NH against Mayer-wave detuning and record entrainment threshold",
            "record metabolic subcriticality boundary as a labelled control channel",
        ),
    },
}
...
class NeurovascularValidationSpec:
    """Validation spec promoted from Paper 0 neurovascular source anchors."""
...
@dataclass(frozen=True, slots=True)
class NeurovascularValidationSpecBundle:
    """Neurovascular validation specs plus coverage summary."""

    specs: tuple[NeurovascularValidationSpec, ...]
...
def build_neurovascular_validation_specs(
    source_records: list[dict[str, Any]],
) -> NeurovascularValidationSpecBundle:
    """Build source-covered validation specs for the Paper 0 neurovascular family."""
    ...
    specs: list[NeurovascularValidationSpec] = []
    consumed_ids: set[str] = set()
    for key in ("embodied.neurovascular_phase_coupling",):
    ...
    summary = {
        ...
        "hardware_status": "simulator_only_no_provider_submission",
        "policy": (
            "Neurovascular anchors are promoted only into validation specifications. "
            "Provider submission remains out of scope until simulator fixtures, null "
            "controls, source-to-result provenance, and biomedical boundary labels pass."
        ),
    }
    return NeurovascularValidationSpecBundle(specs=tuple(specs), summary=summary)


def build_validation_report(bundle: NeurovascularValidationSpecBundle) -> str:
    """Render a concise Markdown report for neurovascular validation specs."""
    status = "match" if bundle.summary["coverage_match"] else "mismatch"
    lines = [
        "# Paper 0 Neurovascular Validation Specs",
        "",
...
def write_outputs(
    bundle: NeurovascularValidationSpecBundle,
    *,
    output_dir: Path = DEFAULT_EXTRACTION_DIR,
    date_tag: str = "2026-05-13",
) -> dict[str, Path]:
    """Write JSON and Markdown outputs for the neurovascular spec bundle."""
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / f"paper0_neurovascular_validation_specs_{date_tag}.json"
    report_path = output_dir / f"paper0_neurovascular_validation_specs_report_{date_tag}.md"
    payload = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    bundle = build_neurovascular_validation_specs(records)
