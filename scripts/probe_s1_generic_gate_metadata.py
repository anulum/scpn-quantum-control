...
# SCPN Quantum Control — probe s1 generic gate metadata script
# scpn-quantum-control -- S1 generic gate metadata probe
"""No-submit S1 generic gate-backend metadata capability probe."""
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Probe S1 dynamic-circuit capability from provider-neutral gate metadata "
            "without network access or job submission."
        )
    )
    parser.add_argument("metadata_json", type=Path, help="Provider-neutral metadata JSON.")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=OUT_DIR,
...
def build_decision_document(metadata: Mapping[str, Any]) -> dict[str, Any]:
    """Build a no-submit generic gate capability decision document."""
    snapshot = snapshot_from_generic_metadata(metadata)
    package = _package()
    decision = assess_feedback_backend_capability(snapshot, package)
    return {
        "date": DATE,
        "script": "scripts/probe_s1_generic_gate_metadata.py",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    document = build_decision_document(metadata)
    args.out_dir.mkdir(parents=True, exist_ok=True)
    backend_name = document["capability_decision"]["backend_name"]
    out_path = args.out_dir / f"s1_generic_gate_metadata_probe_{backend_name}_{DATE}.json"
