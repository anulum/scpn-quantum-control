...
# SCPN Quantum Control — Multi-tier benchmark for the mean-field force and Jacobian
...
This script is the authoritative source for the mean-field chain ordering recorded in
``src/scpn_quantum_control/accel/kuramoto_mean_field.py``. A tier that is not installed is
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark mean-field force/Jacobian across tiers"
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/mean_field_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        from scpn_quantum_control.accel.julia import mean_field_force, mean_field_jacobian
    except Exception:
        return False
    try:
        mean_field_force(np.zeros(8, dtype=np.float64), 1.0)
        mean_field_jacobian(np.zeros(8, dtype=np.float64), 1.0)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    """Run the mean-field tier benchmark CLI for the force and the Jacobian."""
    ...
    from scpn_quantum_control.accel import kuramoto_mean_field as mf

    rust_ok = disp._rust_available()
    julia_ok = disp._julia_available()
    force_tiers: dict[str, _Tier | None] = {
        "rust": partial(mf._rust_mean_field_force, coupling=1.0) if rust_ok else None,
        "julia": partial(mf._julia_mean_field_force, coupling=1.0) if julia_ok else None,
        "python": partial(mf._python_mean_field_force, coupling=1.0),
    }
    jacobian_tiers: dict[str, _Tier | None] = {
        "rust": partial(mf._rust_mean_field_jacobian, coupling=1.0) if rust_ok else None,
        "julia": partial(mf._julia_mean_field_jacobian, coupling=1.0) if julia_ok else None,
        "python": partial(mf._python_mean_field_jacobian, coupling=1.0),
    }
    ...
    payload = {
        ...
        "mean_field_force": _bench_function(
            "mean_field_force (K=1)", force_tiers, sizes, args, rng
        ),
        "mean_field_jacobian": _bench_function(
            "mean_field_jacobian (K=1)", jacobian_tiers, sizes, args, rng
        ),
    }
