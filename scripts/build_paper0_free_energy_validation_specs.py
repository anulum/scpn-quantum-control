...
# scpn-quantum-control -- Paper 0 free-energy spec builder
"""Promote Paper 0 EQ0130-EQ0131 free-energy anchors into specs."""
...
SPEC_METADATA: dict[str, dict[str, tuple[str, ...] | str]] = {
    "computational.variational_free_energy": {
        "validation_protocol": "paper0.computational.free_energy.variational_decomposition",
        "implementation_status": "validation_spec_pending_executable_fixture",
        "null_controls": (
            "support-mismatch control must reject D_KL evaluation",
            "identical-density control must produce zero KL complexity",
            "zero-likelihood control must reject accuracy evaluation",
        ),
        "executable_validation_targets": (
            "verify KL complexity term is non-negative on shared support",
            "verify complexity plus accuracy decomposition equals total F",
            "verify free-energy upper-bound relation against negative log evidence",
        ),
    },
}
...
class FreeEnergyValidationSpec:
    """Validation spec promoted from Paper 0 EQ0130-EQ0131 anchors."""
...
@dataclass(frozen=True, slots=True)
class FreeEnergyValidationSpecBundle:
    """Free-energy validation specs plus coverage summary."""

    specs: tuple[FreeEnergyValidationSpec, ...]
...
def build_free_energy_validation_specs(
    source_records: list[dict[str, Any]],
) -> FreeEnergyValidationSpecBundle:
    """Build source-covered validation specs for EQ0130-EQ0131."""
    ...
    specs: list[FreeEnergyValidationSpec] = []
    consumed_ids: set[str] = set()
    for key in ("computational.variational_free_energy",):
    ...
    summary = {
        ...
        "hardware_status": "simulator_only_no_provider_submission",
        "policy": (
            "EQ0130-EQ0131 anchors are promoted only into validation specifications. "
            "Free-energy and predictive-coding interpretations remain finite "
            "mathematical-boundary checks until executable falsification controls pass."
        ),
    }
    return FreeEnergyValidationSpecBundle(specs=tuple(specs), summary=summary)


def build_validation_report(bundle: FreeEnergyValidationSpecBundle) -> str:
    """Render a concise Markdown report for free-energy validation specs."""
    status = "match" if bundle.summary["coverage_match"] else "mismatch"
    lines = [
        "# Paper 0 Free-Energy Validation Specs",
        "",
    ...
    lines.extend(
        [
            ...
            "source-anchored validation specifications only; free-energy and "
            "predictive-coding interpretations require executable controls "
...
def write_outputs(
    bundle: FreeEnergyValidationSpecBundle,
    *,
    output_dir: Path = DEFAULT_EXTRACTION_DIR,
    date_tag: str = "2026-05-13",
) -> dict[str, Path]:
    """Write JSON and Markdown outputs for the free-energy spec bundle."""
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / f"paper0_free_energy_validation_specs_{date_tag}.json"
    report_path = output_dir / f"paper0_free_energy_validation_specs_report_{date_tag}.md"
    payload = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    bundle = build_free_energy_validation_specs(records)
