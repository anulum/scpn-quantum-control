...
# scpn-quantum-control -- Paper 0 Axiom I family predictions fixture runner
"""Run the Paper 0 Axiom I family-predictions source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_family_predictions_fixture_result_2026-05-13.json"
)
DEFAULT_REPORT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_family_predictions_fixture_report_2026-05-13.md"
)
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Axiom I Family Predictions Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Conditional prediction count: {payload['conditional_prediction_count']}",
        f"- Rejected model class count: {payload['rejected_model_class_count']}",
        f"- Blank separator count: {payload['blank_separator_count']}",
        f"- Next source boundary: {payload['next_source_boundary']}",
        f"- Protocol state: {payload['problem_metadata']['protocol_state']}",
        "",
        "## Conditional Predictions",
    ]
    for key, role in payload["conditional_predictions"].items():
        lines.append(f"- `{key}`: `{role}`")
    lines.extend(["", "## Rejected Model Classes"])
    for key, role in payload["rejected_families"].items():
        lines.append(f"- `{key}`: `{role}`")
