...
# scpn-quantum-control -- Paper 0 Axiom I model-class overview fixture runner
"""Run the Paper 0 Axiom I model-class overview source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_model_class_overview_fixture_result_2026-05-13.json"
)
DEFAULT_REPORT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_model_class_overview_fixture_report_2026-05-13.md"
)
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Axiom I Model-Class Overview Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Selection criterion count: {payload['selection_criterion_count']}",
        f"- Model-class choice count: {payload['model_class_choice_count']}",
    ...
    for key, role in payload["selection_criteria"].items():
        lines.append(f"- `{key}`: `{role}`")
    lines.extend(["", "## Model-Class Choices"])
    for key, choice in payload["model_class_choices"].items():
