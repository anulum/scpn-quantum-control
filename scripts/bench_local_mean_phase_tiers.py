...
in ``src/scpn_quantum_control/accel/local_phase.py``. A tier that is not installed is
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark network-local mean phase/Jacobian across tiers"
    )
    parser.add_argument(
        "--sizes",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("docs/benchmarks/local_mean_phase_tiers.json"),
...
def _warm_julia() -> bool:
    ...
    try:
        ...
        local_mean_phase(theta, coupling)
        local_mean_phase_jacobian(theta, coupling)
        return True
    except Exception:
        return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    from scpn_quantum_control.accel import local_phase as lp

    rust_ok = disp._rust_available()
    julia_ok = disp._julia_available()
    value_tiers: dict[str, _Tier | None] = {
        "rust": lp._rust_local_mean_phase if rust_ok else None,
        "julia": lp._julia_local_mean_phase if julia_ok else None,
        "python": lp._python_local_mean_phase,
    }
    jacobian_tiers: dict[str, _Tier | None] = {
        "rust": lp._rust_local_mean_phase_jacobian if rust_ok else None,
        "julia": lp._julia_local_mean_phase_jacobian if julia_ok else None,
        "python": lp._python_local_mean_phase_jacobian,
    }
    ...
    payload = {
        ...
        "local_mean_phase": _bench_function("local_mean_phase", value_tiers, sizes, args, rng),
        "local_mean_phase_jacobian": _bench_function(
            "local_mean_phase_jacobian", jacobian_tiers, sizes, args, rng
        ),
    }
