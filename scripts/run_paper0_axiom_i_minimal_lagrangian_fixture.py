...
# scpn-quantum-control -- Paper 0 Axiom I minimal Lagrangian fixture runner
"""Run the Paper 0 Axiom I minimal Lagrangian source-accounting fixture."""
...
DEFAULT_OUTPUT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_minimal_lagrangian_fixture_result_2026-05-13.json"
)
DEFAULT_REPORT_PATH = (
    DEFAULT_OUTPUT_DIR / "paper0_axiom_i_minimal_lagrangian_fixture_report_2026-05-13.md"
)
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper 0 Axiom I Minimal Lagrangian Fixture",
        "",
        f"- Source span: {payload['source_ledger_span'][0]} - {payload['source_ledger_span'][1]}",
        f"- Hardware status: {payload['hardware_status']}",
        f"- Claim boundary: {payload['claim_boundary']}",
        f"- Minimal criterion count: {payload['minimal_criterion_count']}",
        f"- Lagrangian operator count: {payload['lagrangian_operator_count']}",
        f"- Equation record count: {payload['equation_record_count']}",
        f"- Next source boundary: {payload['next_source_boundary']}",
        f"- Protocol state: {payload['problem_metadata']['protocol_state']}",
        "",
        "## Minimal Requirements",
    ]
    for key, role in payload["minimal_requirements"].items():
        lines.append(f"- `{key}`: `{role}`")
    lines.extend(["", "## Lagrangian Operators"])
    for key, operator in payload["lagrangian_operators"].items():
        lines.append(f"- `{key}`: `{operator}`")
