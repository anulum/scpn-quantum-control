...
# scpn-quantum-control -- Paper 0 fine-tuning PES fixture runner
"""Run Paper 0 fine-tuning PES simulator fixtures."""
...
def run_default_fixture() -> FineTuningPESFixtureResult:
    """Run default simulator-only fine-tuning PES fixture."""
    return validate_fine_tuning_pes_fixture()
...
def render_report(result: FineTuningPESFixtureResult, runtime_ms: float) -> str:
    """Render a Markdown report for the fine-tuning PES fixture."""
    return "\n".join(
        [
            "# Paper 0 Fine-Tuning PES Fixture",
            "",
            f"- Hardware status: `{result.hardware_status}`",
            f"- Source span: `{', '.join(result.source_ledger_span)}`",
            f"- Selection probability: `{result.selection_probability:.6f}`",
            ...
            "PES, fine tuning, weak-value coupling, retrocausality, or SEC maximisation.",
            "",
        ]
    )
...
def write_outputs(
    result: FineTuningPESFixtureResult,
    *,
    ...
    json_path = output_dir / f"paper0_fine_tuning_pes_fixture_result_{date_tag}.json"
    report_path = output_dir / f"paper0_fine_tuning_pes_fixture_report_{date_tag}.md"
