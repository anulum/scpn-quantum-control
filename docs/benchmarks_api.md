...
| `selection_piecewise_contracts` | selection-heavy | Registry-gated `where`/`clip` branch and boundary contracts, with `np.select`, callable `np.piecewise`, static-selector `np.choose`, and static-mask `np.compress` folded through trace selection/gather semantics |
