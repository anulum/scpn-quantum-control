...
| `program_ad_static_alias_lattice_contracts` | alias-lattice | Static alias-lattice readiness over emitted `program_ad_effect_ir.v1` components with view-alias, bounded local object-attribute, expression-rebinding, explicit mutation-effect blockers, unsupported-Python frontend blockers, non-executed phi blockers, and control-path alias blocker reporting; captured/global object-attribute aliasing, arbitrary dynamic-Python frontend lowering, mutation adjoints, non-executed branch adjoints, Rust/LLVM executable lowering, hardware, and performance promotion remain blocked |
