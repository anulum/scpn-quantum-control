...
| `program_ad_static_alias_lattice_contracts` | alias-lattice | Static alias-lattice readiness over emitted `program_ad_effect_ir.v1` components with view-alias, bounded local object-attribute, expression-rebinding, and explicit non-executed phi blocker reporting; captured/global object-attribute aliasing, non-executed branch adjoints, Rust/LLVM executable lowering, hardware, and performance promotion remain blocked |
