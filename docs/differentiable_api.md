...
| `scpn_quantum_control.phase.jax_bridge` | Signature-stable optional-JAX facade over the result-contract, bounded-gradient, registered-QNode transform, and compatibility leaves. It retains lowering matrices, cloud validation planning, and maturity aggregation while preserving the existing fail-closed capability boundaries. |
