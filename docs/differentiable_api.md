...
| `scpn_quantum_control.fault_tolerant_resource_product` | BL-36 conservative future-resource composition: explicit precision allocation, surface-code distance union bound, rotated-patch/repetition register counts, Clifford+T rotation estimate, sensitivity and non-equivalent regime rows; no FT availability/execution claim. See [Fault-tolerant resource product](fault_tolerant_resource_product.md). |
