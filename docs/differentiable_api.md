...
| `scpn_quantum_control.phase.torch_bridge` | Optional PyTorch execution facade over the result-contract, bounded-gradient, registered-QNode, and compatibility leaves for broad PyTorch module/transform/compiler/device maturity routing, live CPU-overlay external-comparison artefact validation, cloud planning, and a fail-closed registered Phase-QNode Torch-lowering matrix checked against parameter-shift references and promotion blockers. |
