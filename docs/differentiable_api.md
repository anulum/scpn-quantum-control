...
| `scpn_quantum_control.metamorphic_ad_verification` | BL-46 metamorphic AD verification catalogue: law IDs, pure residual checks (linearity/chain rule), evidence-gated suite pointers, anti-silent-wrong and refuse-invent-green formal-proof boundaries. See [Metamorphic AD verification](metamorphic_ad_verification.md). |
...
| `scpn_quantum_control.resource_budget_gate` | BL-94 compile & dense resource budgets: catalogue of Pauli/compile and dense Hilbert dimensions, estimate/check/enforce composing `compile_budget`/`dense_budget`, exceed → typed refuse. See [Resource budget gate](resource_budget_gate.md). |
...
| `scpn_quantum_control.surrogates` | BL-45 Gaussian-RBF classical surrogate with analytic input gradients, disjoint held-out exact-statevector fidelity certificates, and an unapplied BL-33 proposal that requires a separate exact local objective query. It is not an exact quantum-gradient, hardware, generalisation, or control-decision surface. |
