...
| `scpn_quantum_control.hardware_safe_execution` | Hardware-safe gradient execution product: no-submit default policies, dry-run shot plans, enforce refuse for would-submit/over-budget, secret-free audit records. See [Hardware-safe execution](hardware_safe_execution.md). |
