...
| `scpn_quantum_control.governed_route_matrix` | BL-52 fail-closed multi-ecosystem route matrix: unified route-ID taxonomy across transform/adapter/compiler/Rust/provider/competitor families, `explain_route(...)` with rejected alternatives, zero blank cells (`supported` / `permanent_boundary` / `implementation_path` only). See [Governed route matrix](governed_route_matrix.md). |
| `scpn_quantum_control.unsuitable_scenario_registry` | BL-53 unsuitable-scenario + anti-silent-wrong registry: versioned catalogue of refuse paths, pure `probe_unsuitable_scenario(...)` (always fail-closed), competitor silent-wrong fixtures with BL-52 deep links. See [Unsuitable scenario registry](unsuitable_scenario_registry.md). |
| `scpn_quantum_control.analysis.rl_research_governance` | BL-102 disabled-by-default RL-adjacent research policy: preregistration, fixed multi-seed suites, iteration/evaluation budgets, deterministic zero-noise replay, dense-reward gaming warning, and permanent BL-58 pulse refusal. See [RL research governance](rl_research_governance.md). |
