...
| `scpn_quantum_control.advantage_language_protocol` | BL-65 advantage / no-advantage language governor: default no-advantage certificate, protocol catalogue, `probe_advantage_language(...)` refuses ungoverned advantage marketing without inventing green claims. See [Advantage language protocol](advantage_language_protocol.md). |
...
| `scpn_quantum_control.analog_mapping` | BL-35 analog oscillator mapping feasibility: immutable request/profile/report contracts, source-dated static capability sketches, fail-closed topology/control/measurement diagnostics, bounded N<=6 ideal-model versus Lie-Trotter comparison, analytic coupling-scale objective, drift sensitivity, and deterministic evidence writing. Internal compiler-model support never promotes provider, hardware-equivalence, submission, performance, or analog-advantage claims. See [Analog oscillator mapping feasibility](analog_mapping_feasibility.md). |
...
| `scpn_quantum_control.hermetic_reproduction_kit` | BL-55 hermetic external reproduction kit: versioned command manifest, core local-only entries (no QPU), pure SHA-256 digest fail-closed checks, refuse invent-green hardware digests. See [Hermetic reproduction kit](hermetic_reproduction_kit.md). |
