...
| `scpn_quantum_control.jax_nqs_baseline_product` | BL-103 research-only JAX RBM baseline: validates small-system inputs, composes exact enumeration and automatic differentiation, compares `2 <= N <= 6` against dense exact diagonalisation, records JAX environment and a deterministic evidence digest, and binds BL-65 no-advantage. It does not promote the JAX-native scorecard, hardware, scale, or performance claims. See [JAX NQS baseline product](jax_nqs_baseline.md). |
