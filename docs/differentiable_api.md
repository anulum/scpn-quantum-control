...
| Rust Phase-QNode parity kernels | `phase_qnode_fubini_study_metric_rust`, `phase_qnode_computational_basis_fisher_rust`, `phase_qnode_vector_jvp_rust`, `phase_qnode_vector_vjp_rust`, `phase_qnode_hessian_vector_product_rust`, `phase_qnode_complex_derivative_contract_rust` | Optional PyO3 parity surface for the promoted deterministic local metric and directional-transform primitives. The kernels operate on materialised state derivatives, Jacobians, or Hessians and are checked against the Python Phase-QNode APIs. |
