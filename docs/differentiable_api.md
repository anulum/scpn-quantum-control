...
| `scpn_quantum_control.stable_core_product` | BL-81 stable_core experiment model product: schema version policy, JSON round-trip/digest over Problem/Backend/Experiment/Result, fail-closed unknown schemas/ids; narrow durable SemVer-intent (BL-97). See [stable_core product](stable_core_product.md). |
...
| `scpn_quantum_control.visualisation_dashboard_product` | BL-34 fixture-driven visualisation dashboard product: static panel catalogue, secrets scan, order-parameter/energy + gradient-norm probes, refuse live QPU streaming / SaaS. See [visualisation dashboard product](visualisation_dashboard_product.md). |
