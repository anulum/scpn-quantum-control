...
metadata, zero-variance `std` gradients, remaining broad linalg/spectral adjoints, unsafe source-level aliases and non-executed branch
...
`np.linalg.multi_dot` linalg-array output replay, while the historical
