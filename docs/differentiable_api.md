...
| `scpn_quantum_control.phase.qiskit_bridge` | Compatibility facade over Qiskit bridge contracts and local gradients for no-submit Runtime capture builders, freshness-bounded provider evidence assembly, and maturity orchestration while the remaining executable concern is split. |
