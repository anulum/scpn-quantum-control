...
| `scpn_quantum_control.differentiable_transform_algebra` | Executable local metamorphic gate for `grad(vmap(f))`, `vmap(grad(f))`, `jacrev`/`jacfwd`, Hessian symmetry, JVP/VJP duality, linearity, chain rule, sparse/masked parameters, dtype/broadcast replay, and fail-closed unsupported transform boundaries. |
