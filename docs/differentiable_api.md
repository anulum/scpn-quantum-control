...
| `scpn_quantum_control.scorecard_acceptance_engine` | Scorecard acceptance engine: eleven baseline categories with honest `behind_baseline` inventory, fail-closed `promote_scorecard_category` (no invent-green without evidence). See [Scorecard acceptance engine](scorecard_acceptance_engine.md). |
