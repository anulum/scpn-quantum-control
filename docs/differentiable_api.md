...
| `scpn_quantum_control.governed_route_matrix` | Fail-closed multi-ecosystem route matrix: unified route-ID taxonomy across transform/adapter/compiler/Rust/provider/competitor families, `explain_route(...)` with rejected alternatives, zero blank cells (`supported` / `permanent_boundary` / `implementation_path` only). See [Governed route matrix](governed_route_matrix.md). |
| `scpn_quantum_control.unsuitable_scenario_registry` | Unsuitable-scenario and anti-silent-wrong registry: versioned catalogue of refuse paths, pure `probe_unsuitable_scenario(...)` (always fail-closed), and competitor silent-wrong fixtures with governed route deep links. See [Unsuitable scenario registry](unsuitable_scenario_registry.md). |
| `scpn_quantum_control.analysis.rl_research_governance` | Disabled-by-default RL-adjacent research policy: preregistration, fixed multi-seed suites, iteration/evaluation budgets, deterministic zero-noise replay, dense-reward gaming warning, and permanent pulse-execution refusal. See [RL research governance](rl_research_governance.md). |
| `scpn_quantum_control.advantage_language_protocol` | Advantage / no-advantage language governor: default no-advantage certificate, protocol catalogue, `probe_advantage_language(...)` refuses ungoverned advantage marketing without inventing green claims. See [Advantage language protocol](advantage_language_protocol.md). |
| `scpn_quantum_control.neural_operator_baseline_product` | Classical neural-operator baseline composition: verifies committed runner arithmetic/digest, separates training and inference labels, binds the governed no-advantage default, admits public/synthetic datasets, and records challenge-oracle and multimodal-forecasting wiring residuals. See [Neural-operator baseline product](neural_operator_baseline_product.md). |
| `scpn_quantum_control.forecasting` | Multimodal partial-observation forecasting: immutable simulation-only series/graph/event/mask custody, training-only imputation/scaling, a ridge persistence comparator, exact known-simulator Kuramoto-residual scoring, independent split residual intervals, no-submit active-sensing composition, and an unapplied controller proposal. Real EEG/clinical/grid/SCADA/plasma/plant data, hidden-state inference, hardware, safety, control-performance, and deployment claims are excluded. See [Multimodal forecasting under partial observation](multimodal_forecasting.md). |
| `scpn_quantum_control.jax_nqs_baseline_product` | Research-only JAX RBM baseline: validates small-system inputs, composes exact enumeration and automatic differentiation, compares `2 <= N <= 6` against dense exact diagonalisation, records JAX environment and a deterministic evidence digest, and binds the default no-advantage certificate. It does not promote the JAX-native scorecard, hardware, scale, or performance claims. See [JAX NQS baseline product](jax_nqs_baseline.md). |
| `scpn_quantum_control.fault_tolerant_resource_product` | Conservative future-resource composition: explicit precision allocation, surface-code distance union bound, rotated-patch/repetition register counts, Clifford+T rotation estimate, sensitivity, and non-equivalent regime rows; no fault-tolerant availability or execution claim. See [Fault-tolerant resource product](fault_tolerant_resource_product.md). |
| `scpn_quantum_control.analog_mapping` | Analog oscillator mapping feasibility: immutable request/profile/report contracts, source-dated static capability sketches, fail-closed topology/control/measurement diagnostics, bounded N<=6 ideal-model versus Lie-Trotter comparison, analytic coupling-scale objective, drift sensitivity, and deterministic evidence writing. Internal compiler-model support never promotes provider, hardware-equivalence, submission, performance, or analog-advantage claims. See [Analog oscillator mapping feasibility](analog_mapping_feasibility.md). |
| `scpn_quantum_control.metamorphic_ad_verification` | Metamorphic AD verification catalogue: law IDs, pure residual checks (linearity/chain rule), evidence-gated suite pointers, anti-silent-wrong and refuse-invent-green formal-proof boundaries. See [Metamorphic AD verification](metamorphic_ad_verification.md). |
| `scpn_quantum_control.hermetic_reproduction_kit` | Hermetic external reproduction kit: versioned command manifest, core local-only entries (no QPU), pure SHA-256 digest fail-closed checks, refuse invent-green hardware digests. See [Hermetic reproduction kit](hermetic_reproduction_kit.md). |
| `scpn_quantum_control.attested_result_pack` | BL-48 attested result packs: canonical digest over content bound to claim axes, local unsigned envelope, verify statuses VERIFIED/STRIPPED/FORGED/UNGRADED (never silent-validated). See [Attested result pack](attested_result_pack.md). |
| `scpn_quantum_control.scorecard_acceptance_engine` | BL-56 scorecard acceptance engine (W1): eleven baseline categories with honest `behind_baseline` inventory, fail-closed `promote_scorecard_category` (no invent-green without evidence). See [Scorecard acceptance engine](scorecard_acceptance_engine.md). |
| `scpn_quantum_control.public_api_stability` | Public API stability programme: narrow `semver_stable` catalogue vs experimental workbench/internal, deprecation probe + decorator, fail-closed unknown/blank ids, refuse breaking public removals without deprecation. See [Public API stability](public_api_stability.md). |
| `scpn_quantum_control.competitive_baseline_watch` | Continuous competitive-baseline watch: pinned-snapshot inventory, fail-closed refresh and feed probes, no scorecard promotion from watch evidence alone, and pointer-only route-matrix output. See [Competitive baseline watch](competitive_baseline_watch.md). |
| `scpn_quantum_control.hardware_safe_execution` | BL-47 hardware-safe gradient execution product: no-submit default policies, dry-run shot plans, enforce refuse for would-submit/over-budget, secret-free audit records. See [Hardware-safe execution](hardware_safe_execution.md). |
| `scpn_quantum_control.resource_budget_gate` | Compile and dense resource budgets: catalogue of Pauli/compile and dense Hilbert dimensions, estimate/check/enforce composing `compile_budget`/`dense_budget`, exceed → typed refuse. See [Resource budget gate](resource_budget_gate.md). |
| `scpn_quantum_control.qpu_compute_product` | qpu_compute plan product: typed dry-run plan kinds, fail-closed unknown ids, refuse would-live/hardware_enabled, and a composed secret-free hardware-safety audit. See [qpu_compute product](qpu_compute_product.md). |
| `scpn_quantum_control.phase_qnode_product` | Phase-QNode product surface: versioned journey map (build→differentiate→dry-run), support badges, fail-closed unknown IDs, hardware-safe no-submit refusal, and an explicitly experimental workbench rather than a broad SemVer-frozen contract. See [Phase-QNode product](phase_qnode_product.md). |
| `scpn_quantum_control.whole_program_ad_product` | Whole-program AD product surface: versioned frontend→IR→adjoint journey map, architecture layers, fail-closed unknown IDs, unsupported frontends route to the unsuitable-scenario registry, and hardware/polyglot/edge requests remain no-submit or residual; the workbench is not SemVer-frozen. See [Whole-program AD product](whole_program_ad_product.md). |
| `scpn_quantum_control.stable_core_product` | Stable-core experiment model: schema version policy, JSON round-trip and digest over Problem/Backend/Experiment/Result, and fail-closed unknown or drifted contracts; narrow durable SemVer intent through the public API stability programme. See [stable-core product](stable_core_product.md). |
| `scpn_quantum_control.polyglot_parity_certificate` | Bit-exact polyglot parity certificates: deterministic family catalogue, schema v2 digests, build/verify helpers, exact contract validation, and fail-closed unsupported paths; no full NumPy parity claim. See [polyglot parity certificates](polyglot_parity_certificate.md). |
| `scpn_quantum_control.polyglot_edge_ad_product` | Polyglot edge Program-AD runtime contract: capability matrix over bounded native Rust, committed-sample browser WASM, and the unsupported Julia Program-AD boundary; composes the value-and-gradient parity certificate and Studio executive replay route while forbidding silent host fallback. See [polyglot edge AD product](polyglot_edge_ad_product.md). |
| `scpn_quantum_control.program_ad_fuzz_assurance` | BL-96 Rust Program AD fuzz assurance: target catalogue over ambient cargo-fuzz bins, time-boxed CI-optional policy, dry-run refuse invent-green continuous coverage. See [Program AD fuzz assurance](program_ad_fuzz_assurance.md). |
| `scpn_quantum_control.custom_derivatives_product` | Custom/registered derivatives product: versioned registration contract, fail-closed register/query, scaled-linear example rule, transform-algebra interaction coverage boundary, and per-rule metamorphic-verification boundary. See [custom derivatives product](custom_derivatives_product.md). |
| `scpn_quantum_control.stochastic_estimators_product` | Stochastic estimators and policies product: SPSA/score-function/shot-allocation catalogue, confidence policy, and dry-run refusal of hardware shots under the hardware-safe no-submit boundary. See [stochastic estimators product](stochastic_estimators_product.md). |
| `scpn_quantum_control.wirtinger_implicit_product` | Wirtinger and implicit-differentiation product: Wirtinger/holomorphic/CR catalogue, implicit stationary/fixed-point demos, and explicit unsuitable-scenario plus metamorphic-law refusal for complex objectives without a Wirtinger contract. See [Wirtinger + implicit product](wirtinger_implicit_product.md). |
| `scpn_quantum_control.adjoint_replay_product` | Adjoint reversible-simulator replay product: reverse Program AD adjoint and executable step-replay catalogue, checkpoint policy, and fail-closed mid-circuit/Catalyst/hardware boundaries. See [adjoint replay product](adjoint_replay_product.md). |
| `scpn_quantum_control.migration_guides_product` | PennyLane + Qiskit migration guides product: concept-map catalogue, local PennyLane import round trip and Qiskit Statevector parameter-shift demos, with fail-closed live Runtime and full-parity boundaries. See [migration guides product](migration_guides_product.md). |
| `scpn_quantum_control.differentiable_notebook_curriculum` | Core-six differentiable curriculum catalogue under notebooks/differentiable, hardware_execution=false, with fail-closed QPU and archive-conversion boundaries. See [differentiable notebook curriculum](differentiable_notebook_curriculum.md). |
| `scpn_quantum_control.visualisation_dashboard_product` | Fixture-driven visualisation dashboard product: static panel catalogue, secrets scan, order-parameter/energy and gradient-norm probes, with fail-closed live QPU streaming and SaaS boundaries. See [visualisation dashboard product](visualisation_dashboard_product.md). |
| `scpn_quantum_control.studio_executive_product` | Studio executive and coverage-frontier product: verb-to-governed-route catalogue, unsuitable-scenario policy, honesty×answer-rate frontier probe, and refusal of invent-green full coverage or unapproved execution. See [studio executive product](studio_executive_product.md). |
| `scpn_quantum_control.control_stack_compose_product` + `control_stack_runtime_adapters` | Compose-existing-stack product: ownership map plus executable, policy-gated realtime/QAOA-MPC/cosimulation adapters; pulse execution fails closed to the optional pulse-execution boundary; refuse missing policy, hardware submission, invent-green PCS, and realtime_runtime rewrites. See [control stack compose product](control_stack_compose_product.md). |
| `scpn_quantum_control.codesign` | Simulator-first quantum-classical co-design loop: immutable step contracts, exact existing phase objectives, governed planner explanations, optional bounded open-system augmentation, existing control ports, active-sensing/identity/geometry observers, adaptive-information proposal telemetry, explicit latency/safety policies, digest-bound replay, and functional non-isolated evidence. Proposals remain unapplied until the safety policy admits them; hardware, provider submission, and operational plasma control fail closed. See [quantum-classical co-design loop](quantum_classical_codesign_loop.md). |
| `scpn_quantum_control.l16` | Bounded exact-simulator indicator suite and conservative co-design safety routing. The legacy weighted score is a heuristic, not a Lyapunov/PCS/stability certificate; autonomous hardware control is permanently refused. See [bounded L16 director](l16_director_product.md). |
| `scpn_quantum_control.active_sensing_product` | Active-sensing product: Gaussian synthetic information-gain ranking after hardware-safe shot-budget approval, real analytic candidate-design evidence, and co-design observer telemetry; adaptive hardware and NV 20 T promotion fail closed. See [active sensing product](active_sensing_product.md). |
| `scpn_quantum_control.analysis.adaptive_fim_feedback` | Uncertainty-aware batch-level FIM next-experiment proposals: disjoint count witnesses, Wilson intervals, minimum-shot and bounded-delta gates, complete hardware-safe paired-arm budget approval, and co-design observer/unapplied-proposal ports. The legacy point-estimate helper is compatibility-only; hardware, closed-loop efficacy, protection, optimal-policy, and advantage claims fail closed. See [adaptive FIM feedback](adaptive_fim_feedback.md). |
| `scpn_quantum_control.identity_observer_product` | Identity/robustness control observers: real energy-gap, transition, coherence, and optional CHSH adapters with explicit thresholds; numeric violations hold and unsupported requested witnesses abort. See [identity observer product](identity_observer_product.md). |
| `scpn_quantum_control.ssgf_geometry_gradient_product` | SSGF quantum-in-the-loop geometry gradient: frozen ambient inventory, C=1-R cross-surface certificates, bounded latent finite differences with refinement/phase-periodicity checks, functional outer-cycle evidence, and a fail-closed latent parameter-shift boundary. See [SSGF geometry-gradient product](ssgf_geometry_gradient_product.md). |
| `scpn_quantum_control.multi_hal_federation_product` | Multi-HAL provider federation product: capability-true matrix over ambient hardware/hal_* descriptors, hardware-safe no-submit dry-run, and refusal of invent-green live submit or network probing. See [multi-HAL federation product](multi_hal_federation_product.md). |
| `scpn_quantum_control.compiler_boundary_product` | QIR/CUDA-Q compiler boundary product: external-compiler register (supported/adapter/implementation_path/permanent_boundary), ambient Catalyst + LLVM claim-gate probe, and refusal of invent-green CUDA-Q runtime or QIR submission. See [compiler boundary product](compiler_boundary_product.md). |
| `scpn_quantum_control.quantum_sync_challenge_oracle_product` | Quantum Sync Challenge oracle: F1–F4 families plus a hardware-gated schema, metric/baseline catalogues, anti-cheat digests, and an ambient witness-suite probe; advantage invention, unvalidated ranking, and automatic hardware execution fail closed. See [sync challenge oracle product](quantum_sync_challenge_oracle_product.md). |
| `scpn_quantum_control.chimera_control` | Finite synthetic chimera and multiscale control facade: exact Kuramoto–Sakaguchi regimes, nested order parameters, composed analytic cluster targets, unapplied phase proposals, topology-ledger projection, and deterministic evidence. No thermodynamic-limit, biological/EEG, stability, controllability, provider, QPU, hardware, or deployment claim. See [chimera and multiscale control](chimera_multiscale_control.md). |
| `scpn_quantum_control.dla_topology_control` | Exact finite parity-sector JVP/VJP and leakage gradients, a projected synthetic objective, and fail-closed fixed-active-set topology-ledger JVP/VJP rules. Discrete topology, persistent homology, active budget rescaling, connectivity thresholds, full-DLA classification, controllability, error correction, provider, QPU, hardware, and deployment remain unsupported. See [DLA and topology-constrained control](dla_topology_constrained_control.md). |
| `scpn_quantum_control.kyma_mechanism_benchmark_product` | KYMA/KYMA v2 mechanism benchmark product: preregistered suite catalogue, frozen teacher-only design-constant digests, and realisability/non-separability certificates; refuse post-hoc retuning or invented advantage without a protocol id. See [KYMA mechanism benchmark product](kyma_mechanism_benchmark_product.md). |
| `scpn_quantum_control.campaign_harness_product` | Campaign-harness productisation: AppQSim / IQM layout-transfer / closed-loop publication / ambient registry templates with digests and no-submit dry-run probes; live submission and unattested claims fail closed. See [campaign harness product](campaign_harness_product.md). |
| `scpn_quantum_control.cloud_native_deployment_product` | Cloud-native deployment boundary product: batch/worker patterns and a threat model over ambient K8s/Compose dry-run manifests; refuse secret env, always-on QPU, and live cluster creation. See [cloud-native deployment product](cloud_native_deployment_product.md). |
| `scpn_quantum_control.thermo_readiness_product` | Thermodynamics-readiness product: claim-bounded K-sweep and readiness estimates over ambient `thermodynamics.readiness`, plus an FEP research-only inventory; refuse invent-green peak claims, hardware submission, and FEP product promotion. See [thermo readiness product](thermo_readiness_product.md). |
| `scpn_quantum_control.open_system_mcwf_product` | Open-system MCWF completeness product: Lindblad/MCWF surfaces, seeded ensemble certificates, simulation noise I/O over ambient tensor_jump + open_system_objectives; refuse unsupported hardware noise, adjoint Lindblad, and non-Markovian process-tensor claims. See [open-system MCWF product](open_system_mcwf_product.md). |
| `scpn_quantum_control.error_mitigation_product` | Differentiable error-mitigation taxonomy: mitigator differentiability classes and local ZNE/readout probes over ambient mitigation modules; ideal-gradient restoration, live-QPU mitigation, and mandatory mitiq claims fail closed. See [error mitigation product](error_mitigation_product.md). |
| `scpn_quantum_control.geometric_control_product` | Geometric quantum-control product: QFI/McLachlan/QNG catalogue, metric-spectrum diagnostics, and a regularised natural-gradient probe over ambient variational_metric + natural_gradient; advantage-at-criticality and live-QPU claims fail closed. See [geometric control product](geometric_control_product.md). |
| `scpn_quantum_control.pgbo_qgt_product` | PGBO quantum geometric tensor product: QGT/Fubini–Study/Berry curvature on coupling K-space via ambient compute_pgbo_tensor; fail-closed N caps; refusal of invented experimental geometry or live-QPU claims. See [PGBO QGT product](pgbo_qgt_product.md). |
| `scpn_quantum_control.advanced_witnesses_product` | Advanced-witness catalogue for Krylov, OTOC, and classical-shadow estimators with `WitnessEstimate` provenance over ambient analysis modules; hard qubit/shot caps; refuses unsupported OTOC advantage, topology certification, live-QPU, and unrestricted-shadow claims; composes the harmonic synchronisation order parameter. See [advanced witnesses product](advanced_witnesses_product.md). |
...
| `scpn_quantum_control.phase.synchronisation_witness` | Bounded synchronisation witnesses over synthetic phase clouds: harmonic Kuramoto order parameters and exact Vietoris-Rips persistent homology (Betti curves and dimension-0/1 persistence diagrams) over geodesic phase distances, with bootstrap order-parameter uncertainty, provenance, and synchronised/desynchronised/clustered reference cases. See [synchronisation witnesses](synchronisation_witnesses.md) and the [API contract](api/synchronisation_witnesses.md). |
...
| `scpn_quantum_control.ml_examples` | Deterministic local convergence certificates for one frozen QNN, QGNN, and QSNN task; complete framework-status rows; notebook pointers; and digest-bound JSON/Markdown evidence. It reuses the existing trainers and makes no arbitrary-architecture, generalisation, provider, QPU, neuromorphic-hardware, or production claim. See [QNN, QGNN, and QSNN convergence examples](ml_convergence_examples.md). |
...
| `scpn_quantum_control.phase.gradient_tape` | Context-managed recording of supported deterministic and finite-shot quantum-gradient evaluations, replay fingerprints, mutation guards, nested/persistent tape contract checks, and fail-closed audit evidence. |
...
| `scpn_quantum_control.surrogates` | Gaussian-RBF classical surrogate with analytic input gradients, disjoint held-out exact-statevector fidelity certificates, and an unapplied co-design proposal that requires a separate exact local objective query. It is not an exact quantum-gradient, hardware, generalisation, or control-decision surface. |
...
| `scpn_quantum_control.benchmarks.differentiable_external_comparison` | External comparison rows and JSON artefact writing for JAX `value_and_grad`/`vmap` support, PyTorch `torch.func`, TensorFlow `GradientTape`, PennyLane QNodes, optional LLVM/Enzyme runner AD, optional Catalyst qjit/MLIR/QIR runner workflows, and a Catalyst boundary row for adaptive finite-shot trainability dry-runs where broadcast/vmap support is not claimed. Rows use strict JSON, timeout, toolchain, correctness gates, dependency-version metadata, dedicated `catalyst_comparison` payloads, explicit `closure_status`/`closure_reason` fields, and explicit hard-gap rows for unsupported batching, transform, dtype, and hardware-device routes. |
...
| `scpn_quantum_control.benchmarks.open_system_objective_evidence` | JSON/Markdown evidence writer for bounded open-system objective rows under `functional_non_isolated` classification. It records Lindblad invariant certificates, MCWF same-seed trajectory replay, finite-difference gradients, boundary rows, and environment metadata without promoting adjoint, provider, hardware, stochastic-gradient, or isolated benchmark claims. |
| `scpn_quantum_control.benchmarks.coupling_recovery_evidence` | JSON/Markdown evidence writer for bounded Kuramoto/XY coupling time-series recovery rows under `functional_non_isolated` classification. It records known ground truth, learned couplings, error metrics, rank/conditioning, noisy/missing-data metadata, boundary rows, and environment metadata without promoting hardware Hamiltonian learning, provider execution, isolated timing, or arbitrary partial-observation inference. |
| `scpn_quantum_control.benchmarks.sync_witness_evidence` | JSON/Markdown evidence writer for bounded synchronisation-witness rows under `functional_non_isolated` classification. It records harmonic order parameters, bootstrap uncertainty, Betti-0/1 curves, dimension-0/1 persistence diagrams, persistent component counts, dominant H1 lifetimes, boundary rows, and environment metadata without promoting hardware phase tomography, provider execution, isolated timing, or high-dimensional manifold inference. |
...
The Rust dynamic-boundary battery exercises those blocked indexing,
...
`run_gradient_tape_contract_audit()` to verify the contract checks for
