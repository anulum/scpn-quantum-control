...
| `scpn_quantum_control.stochastic_estimators_product` | BL-93 stochastic estimators & policies product: SPSA/score-function/shot-allocation catalogue, confidence policy, dry-run refuse hardware shots (BL-47). See [stochastic estimators product](stochastic_estimators_product.md). |
...
| `scpn_quantum_control.cloud_native_deployment_product` | BL-101 cloud-native deployment boundary product: batch/worker patterns + threat model over ambient K8s/Compose dry-run manifests; refuse secret env / always-on QPU / live cluster create. See [cloud-native deployment product](cloud_native_deployment_product.md). |
