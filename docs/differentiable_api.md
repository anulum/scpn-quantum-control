...
| `scpn_quantum_control.phase.qnode_circuit` | Executable facade over the shared contract and builder leaves for arbitrary-depth registered circuit validation, support analysis, gate-aware parameter-shift planning and gradients, statevector/density execution, exact and finite-shot Fisher metrics, QFI/Fubini-Study metrics, natural-gradient providers, and numerical state/density/observable kernels. |
