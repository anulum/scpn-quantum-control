...
| `scpn_quantum_control.codesign` | BL-33 simulator-first quantum-classical co-design loop: immutable step contracts, exact existing phase objectives, BL-09 planner explanations, optional BL-16 augmentation, BL-67 control ports, BL-68/69/70 observers, explicit latency/safety policies, digest-bound replay, and functional non-isolated evidence. Hardware, provider submission, and operational plasma control fail closed. See [quantum-classical co-design loop](quantum_classical_codesign_loop.md). |
