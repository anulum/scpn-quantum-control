...
| `scpn_quantum_control.ssgf_geometry_gradient_product` | BL-70 SSGF quantum-in-the-loop geometry gradient: frozen ambient inventory, C=1-R cross-surface certificates, bounded latent finite differences with refinement/phase-periodicity checks, functional outer-cycle evidence, and a fail-closed latent parameter-shift boundary. See [SSGF geometry-gradient product](ssgf_geometry_gradient_product.md). |
