...
| `scpn_quantum_control.phase.qnode_circuit` | Executable facade over the shared contract leaf for the registered local Phase-QNode statevector and density-matrix family: controlled-gate decomposition, arbitrary-depth and multi-qubit template builders, sparse Ising construction, support analysis, gate-aware parameter-shift planning and gradients, exact and finite-shot Fisher metrics, QFI/Fubini-Study metrics, natural-gradient providers, and numerical state/density/observable kernels. |
