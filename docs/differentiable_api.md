...
`concatenate`/`stack`, static-axis `sum`/`mean`/`prod` reductions, and scalar
all-axis `sum`/`mean`/`prod` objective closure, plus static source-map
...
mutation, non-lowered dynamic indexing semantics, variance/std and
