...
| `scpn_quantum_control.polyglot_edge_ad_product` | BL-74 polyglot edge Program-AD product: capability matrix over bounded native Rust, committed-sample browser WASM, and the unsupported Julia Program-AD boundary; composes the BL-49 value+gradient certificate and Studio executive replay route while forbidding silent host fallback. See [polyglot edge AD product](polyglot_edge_ad_product.md). |
...
| `scpn_quantum_control.codesign` | BL-33 simulator-first quantum-classical co-design loop: immutable step contracts, exact existing phase objectives, BL-09 planner explanations, optional BL-16 augmentation, BL-67 control ports, BL-68/69/70 observers, BL-80 proposal telemetry, explicit latency/safety policies, digest-bound replay, and functional non-isolated evidence. BL-80 remains unapplied; hardware, provider submission, and operational plasma control fail closed. See [quantum-classical co-design loop](quantum_classical_codesign_loop.md). |
| `scpn_quantum_control.l16` | BL-85 bounded exact-simulator indicator suite and conservative BL-33 safety routing. The legacy weighted score is a heuristic, not a Lyapunov/PCS/stability certificate; autonomous hardware control is permanently refused. See [bounded L16 director](l16_director_product.md). |
| `scpn_quantum_control.active_sensing_product` | BL-68 active-sensing product: Gaussian synthetic information-gain ranking after BL-47 shot-budget approval, real S3 analytic evidence, and BL-33 observer telemetry; adaptive hardware and NV 20 T promotion fail closed. See [active sensing product](active_sensing_product.md). |
...
| `scpn_quantum_control.multi_hal_federation_product` | BL-75 Multi-HAL provider federation product: capability-true matrix over ambient hardware/hal_* descriptors, no-submit dry-run (BL-47), refuse invent-green live submit / network dry-run. See [multi-HAL federation product](multi_hal_federation_product.md). |
| `scpn_quantum_control.compiler_boundary_product` | BL-66 QIR/CUDA-Q compiler boundary product: external-compiler register (supported/adapter/implementation_path/permanent_boundary), ambient Catalyst + LLVM claim-gate probe, refuse invent-green CUDA-Q runtime / QIR submit. See [compiler boundary product](compiler_boundary_product.md). |
...
| `scpn_quantum_control.pgbo_qgt_product` | BL-71 PGBO quantum geometric tensor product: QGT/Fubini–Study/Berry curvature on coupling K-space via ambient compute_pgbo_tensor; fail-closed N caps; refuse invent-green experimental geometry / live QPU. See [PGBO QGT product](pgbo_qgt_product.md). |
