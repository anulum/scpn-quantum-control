...
scalar/static-linalg plus elementwise/static-structural, fixed `multi_dot`, and
2x2 distinct symmetric `eigvalsh` replay. Metadata
...
symmetric `np.linalg.eigvalsh` output replay, static source-map
...
adjoints beyond the 2x2 `eigvalsh` boundary, unsafe source-level aliases and non-executed branch
...
`np.linalg.multi_dot` linalg-array output replay and bounded 2x2 distinct
symmetric `np.linalg.eigvalsh` spectral replay, while the historical
