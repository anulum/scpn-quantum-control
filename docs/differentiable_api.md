...
2x2 distinct symmetric `eigvalsh`, and 2x2 real-distinct `eigvals` replay. Metadata
...
symmetric `np.linalg.eigvalsh` output replay, bounded 2x2 real-distinct
`np.linalg.eigvals` output replay, static source-map
...
adjoints beyond the bounded 2x2 `eigvalsh` and `eigvals` boundaries, unsafe source-level aliases and non-executed branch
...
symmetric `np.linalg.eigvalsh` spectral replay, and bounded 2x2 real-distinct
`np.linalg.eigvals` spectral replay, while the historical
