...
present, explicit PennyLane `interface` and `diff_method` metadata, SHA-256
result and metadata digests, non-hardware execution mode, and
...
interface or diff method drifts from the provider execution chain, and fails
closed when the bundle expires before the review cutoff.
