...
| `scpn_quantum_control.control_stack_compose_product` + `control_stack_runtime_adapters` | BL-67 compose-existing-stack product: ownership map plus executable, policy-gated realtime/QAOA-MPC/cosimulation adapters; pulse execution fails closed to BL-58; refuse missing policy, hardware submission, invent-green PCS, and realtime_runtime rewrites. See [control stack compose product](control_stack_compose_product.md). |
