...
| `scpn_quantum_control.phase.qiskit_bridge` | Compatibility facade over Qiskit bridge contracts for shifted-circuit generation, deterministic local Statevector parameter-shift gradients, finite-shot surrogate uncertainty, no-submit Runtime capture builders, freshness-bounded provider evidence assembly, and maturity orchestration while the remaining executable concerns are split. |
