...
Total promoted Phase 2 QPU use: 992 quantum seconds, or 16.53 minutes.
...
The `n=4` DLA parity asymmetry is reproduced from independently submitted
Phase 2 raw counts on `ibm_kingston`, with same-day readout control. The
first `n=6,8` scaling continuation is mixed: `n=8` preserves a positive
middle-depth sign, while `n=6` has negative significant depths.
