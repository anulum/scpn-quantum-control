...
| Area | Public status |
|---|---|
| Package line | Version `1.1.0`, Python `>=3.11`, Qiskit `>=2.2,<3.0`. |
...
4. **A differentiable computation lane** for supported scalar, vector, and
   ...
   static diagonal gather/scatter, static dense inverse through 7x7, static
   vector and matrix-RHS solve through 7x7, and 2x2 product lowering.
...
| Python modules | 699 Python source modules + 1 Rust crate (177 PyO3 bindings) + Julia tier (`accel/julia/*.jl`) |
...
- [Sparse Kuramoto CPU Path](kuramoto_sparse_cpu.md) — SciPy sparse force/Euler/RK4 route with 1M-node ring scaling evidence
...
- [RL Research Governance](rl_research_governance.md) — disabled-by-default preregistration, seed, budget, deterministic-replay, and no-control gates
