...
`tools/test_typing_policy.json` is the machine-readable policy. The 98-file enforced cohort set
...
tests, and the 40-file `scientific_runtime_contracts` cohort taking the
differentiable family. Neither of those two slices needed annotation work: every
