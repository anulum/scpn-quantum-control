...
`tools/test_typing_policy.json` is the machine-readable policy. The 703-file enforced cohort set
consists of the 27-file `repository_policy` cohort covering coverage,
...
publication gate tests, the 3-file `claim_release_contracts`
entanglement-sync scientific and evidence slice, which also holds the
isolated-benchmark host-readiness gate, and the 103-file
...
tests, and the 570-file `scientific_runtime_contracts` cohort holding the whole differentiable and phase
