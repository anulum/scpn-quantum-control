...
`tools/test_typing_policy.json` is the machine-readable policy. The 137-file enforced cohort set
...
tests, and the 79-file `scientific_runtime_contracts` cohort holding the whole
differentiable family — every file of it that passes strict mypy, added in two
slices under the policy's 40-file cap. Neither of those two slices needed annotation work: every
