...
scalar/static-linalg plus compact interpolation, compact signal, compact stencil, compact cumulative, elementwise/static-structural, static `matrix_power`, fixed `multi_dot`,
...
static integer-exponent `np.linalg.matrix_power` output replay,
...
`eigh`, `eigvals`, static integer `matrix_power`, SVD singular-value,
rank-1/Nx2/2xN `pinv`, and on-diagonal `diagflat` boundaries,
