...
| Primitive registry | Available for derivative, batching, lowering, shape, dtype, and nondifferentiability contracts on supported primitive identities. Static selection folds, strict sort, `where`, and `clip` are surfaced to dashboards through `program_ad_selection_primitives` backed by `selection_piecewise_contracts`; dynamic masks/selectors, ties, integer-output selector differentiation, Rust/LLVM executable lowering, hardware, and performance promotion remain blocked. | `scpn_quantum_control.differentiable` |
