...
| QNN/QGNN/QSNN training lane | A bounded phase-QNN binary classifier and QSNN parameter-shift training evidence are available locally; arbitrary QNN/QGNN/QSNN stacks and production convergence notebooks remain planned. | [Differentiable API](differentiable_api.md), [Quantum Gradients](quantum_gradients.md) |
...
| Train a bounded QNN classifier | `phase.qnn_training` -> `train_parameter_shift_qnn_classifier(...)` -> [Quantum Gradients](quantum_gradients.md) |
...
5. broader QNN/QGNN/QSNN convergence notebooks beyond the bounded local phase-QNN and QSNN tests;
...
- arbitrary quantum neural architectures beyond the bounded local phase-QNN classifier and declared QSNN training routes;
