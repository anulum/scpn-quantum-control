...
| QNN/QGNN/QSNN training lane | A bounded phase-QNN binary classifier, QNN-specific finite-difference gradient verification, named external-gradient agreement records, and QSNN parameter-shift training evidence are available locally; arbitrary QNN/QGNN/QSNN stacks and production convergence notebooks remain planned. | [Differentiable API](differentiable_api.md), [Quantum Gradients](quantum_gradients.md) |
...
| Train and verify a bounded QNN classifier | `phase.qnn_training` -> `train_parameter_shift_qnn_classifier(...)` -> `verify_parameter_shift_qnn_classifier_gradient(...)` -> [Quantum Gradients](quantum_gradients.md) |
...
5. broader QNN/QGNN/QSNN convergence notebooks beyond the bounded local phase-QNN verifier and QSNN tests;
...
- arbitrary quantum neural architectures beyond the bounded local phase-QNN classifier, its QNN-specific gradient verifier, and declared QSNN training routes;
