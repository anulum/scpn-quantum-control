...
| Compiler/program AD | Available for supported scalar, vector, and matrix primitives with registry contracts, lowering reports, native executable kernels on bounded paths, verified whole-program determinant lowering through `19x19`, and explicit whole-program Python-semantics diagnostics. Closures, default arguments, keyword-only parameters, `*args`, `**kwargs`, and generator expressions are reported as accepted semantics; materialized comprehensions, captured object/dataclass attributes, recursion, generator functions, context managers, exception control flow, and decorators fail closed before execution. | [Differentiable API](differentiable_api.md), [Quickstart](quickstart.md) |
