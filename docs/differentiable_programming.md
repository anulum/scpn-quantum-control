...
shaped-array, compact interpolation, compact signal, compact cumulative, and bounded static structural
...
`transpose`, static-axis `concatenate`/`stack`, compact `signal:` rows, compact `cumsum`/`cumprod`/`diff` rows, static-axis
