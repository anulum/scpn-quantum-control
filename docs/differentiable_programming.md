...
| Parameter-shift gradients | Available for callable scalar objectives, structured `PhaseVQE` gradients, local gradient-descent VQE examples, metric-aware natural-gradient descent, multi-start optimizer comparison evidence, compatible composed phase-control objectives through `scpn_quantum_control.phase`, and finite-shot uncertainty records that must reconstruct their published gradient and covariance from plus/minus shifted samples. | [Quantum Gradients](quantum_gradients.md), [Variational Methods](variational.md) |
