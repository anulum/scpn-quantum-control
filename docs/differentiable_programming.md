...
| JAX, PyTorch, TensorFlow adapters | Optional parameter-shift value-and-gradient bridges, bounded phase-QNN native framework routes, deterministic registered local Phase-QNode statevector value-and-gradient plus flat and PyTree native-transform lowering for JAX, including PyTree Hessian symmetry evidence, deterministic registered local Phase-QNode statevector lowering for PyTorch, plus fail-closed ML parity audits are available for supported phase objectives; native framework autodiff through provider, finite-shot, dynamic-circuit, hardware, and arbitrary-simulator routes remains open. | [Differentiable Roadmap](differentiable_roadmap.md), [Quantum Gradients](quantum_gradients.md) |
