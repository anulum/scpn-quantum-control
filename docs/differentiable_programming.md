...
| Train and verify a bounded QNN classifier | `phase.qnn_training` -> `train_parameter_shift_qnn_classifier(...)` -> `verify_parameter_shift_qnn_classifier_gradient(...)` -> `run_parameter_shift_qnn_conformance_suite(...)` -> [Quantum Gradients](quantum_gradients.md) |
...
5. broader QNN/QGNN/QSNN convergence notebooks beyond the bounded local phase-QNN conformance suite and QSNN tests;
...
- arbitrary quantum neural architectures beyond the bounded local phase-QNN classifier, its QNN-specific gradient verifier, its conformance suite, and declared QSNN training routes;
