...
2x2 distinct symmetric `eigvalsh`, and 2x2 real-distinct `eigvals` replay. Rust
...
remaining broad linalg/spectral adjoints beyond the bounded 2x2 `eigvalsh` and
`eigvals` boundaries,
...
2x2 distinct symmetric `np.linalg.eigvalsh` spectral replay, and bounded 2x2
real-distinct `np.linalg.eigvals` spectral replay. The legacy
