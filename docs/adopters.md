...
| `scpn-control` | Plasma / disruption control. Uses `scpn_quantum_control.control.q_disruption_iter` as the ITER 11-feature quantum backend behind a fail-closed CONTROL facade. The backend advertises the bridge dependency contract, classifier API, Qiskit core dependencies, optional provider families, report schemas, feature ordering, and non-admission policy via `scpn_control_bridge_dependency_contract()`. | `src/scpn_quantum_control/control/q_disruption_iter.py` + `scpn_control_bridge_dependency_contract()`. |
