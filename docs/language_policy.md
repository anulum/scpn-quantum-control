...
| **JAX** | Auto-differentiable physics, XLA JIT compilation, batched parameter sweeps. | `hardware/jax_accel.py`; `accel/jax_kuramoto.py` (the opt-in Kuramoto RK4 autodiff + ensemble tier); `accel/jax_kuramoto_integrators.py` (the opt-in Euler/DOPRI/inertial/symplectic/noisy trajectory tier); `accel/jax_kuramoto_delayed.py` (the delayed-Kuramoto method-of-steps autodiff + GPU tier); `accel/jax_kuramoto_mpc.py` (the differentiable-model receding-horizon MPC tier). |
...
| `accel/kuramoto_visualisation.py` | **No** | **Exempt** | — | — | — | Matplotlib rendering layer: composes figure artists from already-computed trajectory and observable arrays (reusing the shipped `order_parameter` and `community_order_parameters`). No compute hot loop of its own. |
| `accel/jax_kuramoto.py` | Yes | — (JAX tier) | — | — | — | Opt-in JAX RK4 autodiff + ensemble tier (the compiled path here is JAX/XLA, not Rust). Verified against the Rust default forward and hand-derived adjoint. See `docs/kuramoto_jax_tier.md`. |
| `accel/jax_kuramoto_integrators.py` | Yes | — (JAX tier) | — | — | — | Opt-in JAX trajectories for Euler, DOPRI, inertial, symplectic-inertial, and noisy Kuramoto models. The v2 benchmark artefact records parity against the production integrators. See `docs/kuramoto_jax_tier.md`. |
| `accel/jax_kuramoto_delayed.py` | Yes | — (JAX tier) | — | — | — | Opt-in JAX autodiff + GPU tier for the delayed (DDE) method-of-steps integrator (the compiled path here is JAX/XLA, not Rust; the delayed integrator has no Rust/Julia tier). Verified machine-precision-faithful to the NumPy delayed forward and to the hand-derived delayed sensitivity. See `docs/kuramoto_jax_delayed_tier.md`. |
| `accel/jax_kuramoto_mpc.py` | Yes | — (JAX tier) | — | — | — | Opt-in JAX differentiable-model receding-horizon MPC (the compiled path here is JAX/XLA, not Rust). The control-sequence gradient from `jax.value_and_grad` is verified machine-precision-faithful to the hand-derived discrete adjoint `network_control_value_and_grad` at `r*=0`; the plant it drives is the production controlled integrator. See `docs/kuramoto_mpc_tier.md`. |
