...
| **JAX** | Auto-differentiable physics, XLA JIT compilation, batched parameter sweeps. | `hardware/jax_accel.py`; `accel/jax_kuramoto.py` (the Kuramoto RK4 autodiff + GPU tier); `accel/jax_kuramoto_delayed.py` (the delayed-Kuramoto method-of-steps autodiff + GPU tier). |
