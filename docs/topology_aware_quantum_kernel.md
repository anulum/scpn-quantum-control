...
Matrix-axis and dataset identifiers are stripped of surrounding whitespace before
uniqueness checks. Aliases such as `a` and ` a ` cannot name different samples;
dataset train/test disjointness is checked on the same normalized identities.
Both fidelity and RBF generators hash these normalized identifiers, so equivalent
whitespace variants have identical stored identities and content digests. Hashes
for already canonical identifiers are unchanged. Recompute historical matrices
created with padded identifiers before comparing their custody digests.

Binary labels and predictions are validated before integer conversion in dataset
records, evaluation records, classifier fitting and scoring. Only exact real
`-1` and `+1` values are accepted; fractional values, booleans, strings, complex
values and nonfinite values are rejected. Exact floating-point `-1.0` and `1.0`
remain losslessly supported. Invalid evidence is never rounded into a valid label.
