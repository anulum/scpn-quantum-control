...
|---|---|---|
| Package line | Version `1.1.0`, Python `>=3.11`, Qiskit `>=2.2,<3.0`. | `pyproject.toml`, `CHANGELOG.md` |
...
| Mitigated Bell re-run (KIMI-9) | `ibm_fez` Heron r2 | 2026-07 | Promoted preregistered hardware dataset with raw-count reproducer | `data/bell_rerun_mitigated/` (readiness, raw counts, analysis; SHA-256 in files), `docs/campaigns/bell_rerun_mitigated_prereg_2026-07-16.md`, `scripts/submit_bell_rerun_mitigated.py` | Per pair with exact σ: S = 2.6953 ± 0.0231 (q0–q1), S = 2.6807 ± 0.0232 (q2–q3) unmitigated; 2.7517/2.7510 mitigated. March setting-1 anomaly not reproduced (correct sign, near-band magnitude); small residual q2–q3 asymmetry documented. 40 usage seconds; no loophole-free, device-independent, or QKD claim. |
| Maximum-width Kuramoto-XY sweep (WIDTH-1) | `ibm_fez` Heron r2 | 2026-07 | Promoted preregistered hardware dataset with raw-count reproducer and exact MPS baseline | `data/max_width_kuramoto_xy/` (readiness, raw counts, analysis; SHA-256 in files), `docs/campaigns/max_width_kuramoto_xy_prereg_2026-07-16.md`, `scripts/submit_max_width_kuramoto_xy.py` | Workload-width engineering and noise characterisation only: R(n) vs the exact MPS baseline (≈0.61) at n = 32/64/104/122 gives mitigated R = 0.459/0.342/0.346/0.376 (Trotter reps 1; reps 2 similar), error-aware DFS chain layouts, per-qubit readout mitigation on/off. Widest executed workload in this repository (122 qubits, transpiled depth 2103). Shallow 1-D circuits are classically simulable by construction — NO advantage claim. 38 usage seconds. |
| On-QPU dynamic-circuit feedback demo (RC-1) | `ibm_fez` Heron r2 | 2026-07 | Promoted preregistered hardware dataset with raw-count reproducer | `data/onqpu_dynamic_feedback/` (readiness incl. the superseded miscalibrated-gate run, raw counts, analysis; SHA-256 in files), `docs/campaigns/onqpu_dynamic_feedback_prereg_2026-07-16.md`, `scripts/submit_onqpu_dynamic_feedback.py` | Vendor-executed dynamic circuits (mid-circuit measurement, conditional reset and correction) accepted and executed; feedback-on vs matched open-loop TVD = 0.0693 ± 0.0086 (2 rounds) and 0.1477 ± 0.0093 (3 rounds, independent-bin multinomial σ), per-round monitor trigger rates 3–8%. Controller decisions run inside the vendor runtime: NOT external-FPGA control, and no latency claim is derivable from the outputs. 11 usage seconds. |
| Matched-basis QBER re-run with basis metadata (KIMI-8) | `ibm_fez` Heron r2 | 2026-07 | Promoted preregistered hardware dataset with raw-count reproducer and committed per-pub basis metadata | `data/qkd_qber_basis_metadata/` (readiness, raw counts, analysis; SHA-256 in files), `docs/campaigns/qkd_qber_basis_metadata_prereg_2026-07-17.md`, `scripts/submit_qkd_qber_basis_metadata.py` | Matched-basis mismatch of two Bell pairs, job `d9cosfcjeosc73fgikgg`: raw 1.39%/2.71% (ZZ, q0–q1/q2–q3) and 1.68%/3.00% (XX); 0.51%/1.09% and 0.74%/1.33% with full-basis readout mitigation (binomial σ 0.18–0.27% at 4096 shots). The committed naive sift of the March counts under explicitly assumed bases gives 1.95–3.70%; the new mitigated rates fall below that band and far below the published 5.5%/5.8%, whose non-derivability caveat stands. Fixed per-pub bases, no per-shot randomness — NO QKD security, key-rate, or viability claim. 38 usage seconds. |
| March campaign job-id verifiability (KIMI-7) | `ibm_fez` Heron r2 | 2026-07 | Published hash commitments plus dated IBM retrieval receipts for the March 2026 job identifiers | `data/march_flagship_verifiability/march_job_id_commitments.json` (sha256 `cdeba2530ce892377dd3d9902d6faf19e96cc92bdb781f67da7c2bc5721c5309`), `data/march_flagship_verifiability/march_retrieval_receipts_20260717T011035Z.json` (sha256 `55169b36d75877e6dc54e1ce2cc4141d6ef38faa12a72e34e3b56e665accd360`), `scripts/build_march_job_id_commitments.py`, `scripts/retrieve_march_flagship_receipts.py` | Every March artefact label (24 unique job identifiers across `results/ibm_hardware_2026-03-18/`, `results/ibm_hardware_2026-03-28/`, `results/ibm_hardware_2026-03-29/`, `results/march_2026/`) carries a published SHA-256 commitment of its raw IBM job identifier (nonce-blinded; the disclosable `(raw id, nonce)` pair verifies it), and a 2026-07-17 read-only retrieval receipt: 24/24 jobs report `DONE` on `ibm_fez` with creation dates 2026-03-18 to 2026-03-29 and 160.0 IBM-reported usage seconds in total. Verifiability evidence only — no new physics claim; 0 quantum seconds consumed. **Full disclosure 2026-07-18 (AUD-10):** all 24 commitments are now OPENED — `data/march_flagship_verifiability/march_job_id_disclosure_2026-07-18.json` publishes each `(raw IBM job id, nonce)` pair, and `scripts/disclose_ibm_job_ids.py` (+ its CI test) re-verifies `SHA-256("<raw>:<nonce>") == commitment_sha256` for all 24. The nonce-blinding is retired for the flagship; the HMAC salt and API token stay private (they grant nothing once the IDs are open). |
| Flagship TN classical baselines (KIMI-11) | local CPU | 2026-07 | Committed classical-simulability quantification for the flagship workloads | `data/tn_baseline_flagship/tn_baseline_flagship_20260717T024000Z.json` (sha256 `f46f61e6336f9eda03e9e5b077995f90c1cdf80f1f5c3ed39aa9f28cd8bb18cd`), `scripts/tn_baseline_flagship_workloads.py` | Bounded-χ Aer MPS on the identical logical Trotter bodies of the 16-qubit UPDE (t=0.05, 1 rep) and 8-oscillator Kuramoto (t=0.1, 2 reps) workloads versus the exact statevector reference: UPDE-16 reproduces R to 6.3e-4 already at χ=2 (χ=32 → 1.2e-9, <1 s wall) and Kuramoto-8 converges at χ=4 (1.4e-4). The repository has never claimed quantum advantage for these workloads; this row is the committed gPEPS-style-rebuttal defence stating their classical simulability with concrete numbers. No hardware-quality statement follows. |
| Recovered IBM v2 fair experiments | `ibm_fez` Heron r2 | 2026-03 run / 2026-07 recovered | Promoted recovered raw-count hardware dataset (full disclosure) | `data/ibm_hardware_v2_recovered_2026-07-18/` (raw counts, real IBM job identifiers, dated calibration; SHA-256 in `manifest.json`), `scripts/recover_ibm_v2_raw_counts.py`, `scripts/analyse_ibm_v2_recovered.py` | Read-only re-retrieval (0 QPU s) of the nine 2026-03-29 `ibm_fez` jobs originally committed aggregate-only. Full public disclosure: raw per-pub counts + **real** IBM job identifiers + the dated calibration snapshot. The committed reproducer recomputes 8/9 experiment aggregates to \|Δmean\|<1e-4 from the raw counts (A_odd to ~3.7%, original mitigation not in-pack). `F_FIM=0.9158 > F_XY=0.8484` (all-zero survival) is a genuine observation but carries NO coherence-protection claim — that hypothesis is the falsified promoted `ibm_kingston` SCPN/FIM result. Supersedes the quarantined `results/ibm_hardware_v2_2026-03-29/`. |
...
|---|---|---|
| `results/ibm_hardware_v2_2026-03-29/` | **RECOVERED 2026-07-18 → superseded by the promoted `data/ibm_hardware_v2_recovered_2026-07-18/`.** | The original pack was aggregate-only. The nine jobs have now been re-retrieved read-only from IBM (0 QPU s): raw counts + real IBM job identifiers + dated calibration are public, and a committed reproducer recomputes 8/9 rows to \|Δ\|<1e-4. The data is genuine; only the "DUAL PROTECTION" coherence-protection *interpretation* stays retired (falsified on the promoted `ibm_kingston` SCPN/FIM campaign). |
