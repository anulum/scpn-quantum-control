...
| `data/differentiable_phase_qnode/compiler_evidence_boundary_20260705.json` | Native LLVM/JIT focused-test evidence plus current Enzyme/LLVM scalar, vector, matrix, and loop execution boundary. | Compiler boundary evidence only; not general compiler-AD, alias-activity, isolated benchmark, provider, hardware, GPU, or performance promotion. |
