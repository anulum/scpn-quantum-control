...
| `data/differentiable_phase_qnode/differentiable_dependency_environment_map_20260627.json` | Dependency and environment evidence map over lock profiles, SHA-bound toolchain versions, and local/cloud/provider/hardware/GTX 1060/ML350 route classifications. | Dependency and route provenance only; unlocked GPU and deferred execution rows remain hard gaps, not promotion. |
| `data/differentiable_phase_qnode/differentiable_isolated_benchmark_plan_20260627.json` | Reserved-host batch plan over current non-isolated differentiable benchmark/evidence artefacts, including the compiler-promotion batch gate. | Planning evidence only, not benchmark execution or `isolated_affinity` promotion. |
...
| `data/differentiable_phase_qnode/compiler_evidence_boundary_20260705.json` | Native LLVM/JIT focused-test evidence plus current Enzyme/LLVM scalar, vector, matrix, loop execution, and Program AD alias-activity boundary. | Compiler boundary evidence only; not general compiler-AD, isolated benchmark, provider, hardware, GPU, or performance promotion. |
| `data/differentiable_phase_qnode/compiler_alias_activity_evidence_20260706.json` | Program AD static alias-lattice evidence covering bounded view, list, scalar-rebinding, object-attribute, loop-carried, branch-control, and mutation-version alias activity. | Alias-activity compiler evidence only; not general compiler-AD, isolated benchmark, provider, hardware, GPU, or performance promotion. |
| `data/differentiable_phase_qnode/compiler_promotion_batch_20260706.json` | Non-promotional compiler evidence batch that checksums the compiler boundary, alias-activity, native LLVM/JIT, native whole-program AD, Enzyme/MLIR maturity, and raw Enzyme/MLIR breadth evidence. | Batch assembly evidence only; still blocked by missing isolated compiler benchmark artifact IDs. |
| `data/differentiable_phase_qnode/enzyme_mlir_compiler_ad_breadth_artifact_20260706.json` | Raw 11-case Enzyme/MLIR compiler-AD breadth artifact with passing scalar reverse, vector VJP, matrix VJP, loop, LLVM IR generation, and native Enzyme rows plus explicit hard gaps for missing scalar forward, vector JVP, matrix JVP, MLIR lowering runtime, and Enzyme/MLIR alias-activity raw cases. | Case-level breadth evidence only; non-promotional until isolated benchmark and derived breadth evidence pass. |
| `data/differentiable_phase_qnode/enzyme_mlir_maturity_audit_20260616.json` | Enzyme/MLIR maturity audit with MLIR-runtime correctness evidence, native Enzyme scalar evidence, a linked raw 11-case compiler-AD breadth artifact, and missing derived breadth evidence recorded as hard gaps. Partial breadth captures are represented through complete artifacts with explicit case hard gaps before promotion. | Hard-gap evidence, not Enzyme parity promotion. |
...
| Enzyme/LLVM/MLIR | MLIR-runtime correctness evidence is attached; the local `enzyme`, `opt`, `mlir-opt`, and `clang` command/version snapshot is recorded; bounded native LLVM/JIT focused tests pass under the prescribed selector; native LLVM/JIT crash-safety test IDs are attached; rollback/fallback policy is attached; current LLVMEnzyme execution covers scalar, vector, matrix, and loop kernels with zero gradient error; the raw 11-case Enzyme/MLIR breadth artifact is attached with explicit hard-gap rows; bounded Program AD alias-activity evidence covers static alias-lattice complete and fail-closed cases; the compiler promotion batch is assembled as non-promotional checksum evidence. | Isolated compiler benchmark IDs, derived arbitrary Enzyme/program AD breadth evidence, and the Enzyme-JAX external-comparison runtime gap. |
