...
| Enzyme/LLVM/MLIR | MLIR-runtime correctness evidence is attached; the local `enzyme`, `opt`, `mlir-opt`, and `clang` command/version snapshot is recorded; bounded native LLVM/JIT focused tests pass under the prescribed selector; native LLVM/JIT crash-safety test IDs are attached; rollback/fallback policy is attached; current LLVMEnzyme execution covers scalar, vector, matrix, and loop kernels with zero gradient error; bounded Program AD alias-activity evidence covers static alias-lattice complete and fail-closed cases. | Isolated compiler benchmark IDs, derived arbitrary Enzyme/program AD breadth evidence, and the Enzyme-JAX external-comparison runtime gap. |
