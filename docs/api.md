...
`PrimitiveTransformRule` extends that identity binding with primitive-specific batching rules, executable compiler lowering rules, and lowering metadata. `primitive_contract_for()` returns the registered derivative/batching/lowering/shape/dtype/policy/effect contract view, while `primitive_complete_contract_for()` fails closed until every compiler- and vectorization-facing facet is declared. When `vmap(..., primitive_identity=..., registry=...)` is used, the transform dispatches through the registered batching rule instead of the generic eager loop; missing batching rules fail closed. Compiler AD planning consumes the same lowering metadata and lowering-rule presence for MLIR-runtime/Rust/LLVM status reporting.
