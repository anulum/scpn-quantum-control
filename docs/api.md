...
`PrimitiveTransformRule` extends that identity binding with primitive-specific batching rules, executable compiler lowering rules, and lowering metadata. `primitive_contract_for()` returns the registered derivative/batching/lowering/shape/dtype/policy/effect contract view, while `primitive_complete_contract_for()` fails closed until every compiler- and vectorization-facing facet is declared. Program AD linalg primitives (`scpn.program_ad.linalg:det@1`, `inv@1`, `solve@1`, `matrix_power@1`, and `multi_dot@1`) are also identity-gated through this contract surface before trace execution and expose primitive-specific shape/dtype rules plus deterministic NumPy batching rules for mapped scalar/array outputs. Direct registry derivative kernels are available where the flat-vector primitive signature is unambiguous (`det`, `inv`, and vector-RHS `solve`); `matrix_power` and `multi_dot` remain trace-dispatch contracts until their extra static arguments can be represented safely. Their compiler contracts remain intentionally incomplete until native lowering rules exist. When `vmap(..., primitive_identity=..., registry=...)` is used, the transform dispatches through the registered batching rule instead of the generic eager loop; missing batching rules fail closed. Compiler AD planning consumes the same lowering metadata and lowering-rule presence for MLIR-runtime/Rust/LLVM status reporting.
