...
layered: stable facades for users, workflow guides for common tasks, runtime
contracts for persisted QPU data, and advanced references for subsystem extension.
...
| Pulse to FPGA artifact generation | `scpn_quantum_control.codegen.ultrascale_hls.emit_versioned_hls_artifact`, `verify_hls_artifact_manifest`, `pulse_to_vivado_hls`, and `scripts/export_ultrascale_hls_artifact.py` | Emits a versioned Vivado/Vitis HLS source directory plus `manifest.json` for the decoupled SC-NEUROCORE `hdl_gen.hls_ingest` contract; this repo does not run synthesis, prove timing closure, define board pin placement, or execute FPGA hardware. |
...
| Control scope boundary | `scpn_quantum_control.control.*` plus [Control Scope Boundary](control_scope.md) | Synchronisation-control, FRC schedule scoring, and software-in-loop feedback analysis; not generic pulse-shape optimisation, provider-native pulse calibration, hardware drift compensation, or lab-instrument control. |
...
load_reference_validation_registry(path=DEFAULT_REFERENCE_VALIDATION_PATH)
measure_coverage_frontier_from_certifications(rows_or_ledger=None, registry=None)
...
leaving LLVM/JIT and native-backend promotion fail-closed unless those separate
executable backends are verified. Rust Program AD value+gradient replay for
opcode-bearing static vector- and matrix-RHS `solve` output nodes and static
integer `matrix_power` output nodes is covered by the
bounded `program_ad_effect_ir.v1` replay boundary, not by this MLIR-runtime
lowering claim.
...
FiniteShotSampleProvenance(sample_seed, shot_batch_id, source_class)
ParameterShiftSampleRecord(term_index, parameter_index, parameter_name, trainable, shift, coefficient, plus_value, minus_value, plus_variance, minus_variance, plus_shots, minus_shots, sample_seed, shot_batch_id, source_class, gradient_contribution, variance_contribution)
...
QuantumGradientRejectedMethod(method, reasons, supported_if_requested)
QuantumGradientShotPolicy(finite_shot, requested_shots, planned_shots, defaulted, confidence_level, seed, reasons)
QuantumGradientMethodExplanation(capability, selected_plan, rejected_methods, shot_policy, fallback_path, requested_method, claim_boundary)
TapeGradientRecord(name, kind, plan, result, parameter_fingerprint, replay_fingerprint, contract_notes)
GradientTapeContractCheck(name, status, evidence, blocked_reason=None)
GradientTapeContractAuditResult(checks, passed, claim_boundary=GRADIENT_TAPE_CONTRACT_CLAIM_BOUNDARY)
...
parameter_shift_gradient_with_uncertainty(plus_values, minus_values, plus_variances, minus_variances, plus_shots, minus_shots=None, *, sample_provenance=None, value=0.0, parameters=None, rule=None, confidence_level=0.95, confidence_z=1.959963984540054, failure_policy=None) -> StochasticGradientResult
...
compile_whole_program_frontend(objective) -> WholeProgramCompilerFrontendReport
...
render_enzyme_mlir_compiler_ad_breadth_artifact_markdown(artifact) -> str
write_enzyme_mlir_compiler_ad_breadth_artifact(output_dir, artifact) -> EnzymeMLIRCompilerADBreadthArtifactFiles
...
run_torch_dynamic_shape_export_audit(export_path, replay_cases=None, initial_params=None, tolerance=1e-6) -> PhaseTorchDynamicShapeExportResult
run_torch_aot_autograd_export_audit(features, labels, initial_params, artifact_dir, tolerance=1e-6) -> PhaseTorchAOTAutogradExportResult
...
metadata, undocumented generated-QNode interface or diff-method aliases, and
non-coercive shot metadata: `shots=None` is analytic mode and finite-shot mode
requires an explicit positive integer, not a boolean, float, or numeric string.
Provider-plugin execution artefacts use the same shot-count
...
`QiskitRuntimeQPUProviderEvidenceBundle` is the no-submit bundled attachment
...
named `hard_gap` rows and exposing `failed_case_ids`.
`write_enzyme_mlir_compiler_ad_breadth_artifact(...)` persists the validated
raw artifact as paired JSON and reviewer Markdown files and returns
`EnzymeMLIRCompilerADBreadthArtifactFiles`. The maturity audit can
...
confidence radii. It also requires finite-shot sample provenance with a sample
seed, shot-batch ID, and allowlisted source class before caller-supplied
plus/minus tensors can become accepted replay evidence. This keeps
hardware-calibration gradients honest when finite shot budgets dominate
deterministic numerical error.
...
protocols rather than permitting derivative-losing coercion. Scalar/vector
`np.dot` and `np.vdot` produce traced scalars (including the zero result for
aligned empty vectors); `np.matmul` supports aligned rank-1/rank-2
vector-vector, matrix-vector, vector-matrix, and matrix-matrix products.
`np.clip` accepts exactly the traced value, lower bound, and upper bound;
positional or keyword output buffers are rejected. Empty square matrices retain
NumPy's determinant, inverse, and matrix-power identity shapes in the eager
Python trace runtime. The compact Rust replay and LLVM lowering contracts remain
non-empty, bounded-shape surfaces, so that empty-shape repair does not promote a
new native or performance claim.

The shared dispatcher keeps today's parameter-shift, finite-difference, and complex-step backends
