...
The TN/MPS crossover admission contract is regenerated with:
...
It writes
`data/tn_mps_crossover/tn_mps_crossover_admission_2026-07-08.json` and
`docs/tn_mps_crossover_admission.md`. This artifact admits the N=30-40 row schema
...
claim-boundary, and notes. N=30-40 compute remains owner-gated, and the
...
* **Runner:** `scripts/bench_kuramoto_tiers.py` (measurement core in
  `src/scpn_quantum_control/accel/tier_benchmark.py`).
* **Canonical numbers:** the fixed CI runner, which builds the Rust engine from
  source so every Rust tier is present. The workflow is
  `.github/workflows/benchmark-kuramoto-tiers.yml`
  (`workflow_dispatch` + weekly); the artefact is evidence-only because the
  hosted-runner CPU differs from the declared workstation.
* **Side-by-side comparison:** `docs/tier_benchmarks.md` (generated by
  `tools/render_tier_benchmarks.py`) places the CI and workstation columns next
  to each other for all primitives.
* **Reproducibility manifest:** `docs/benchmarks/tiers/kuramoto_tiers_manifest.<env>.json`
  records the host, toolchain versions (Python, NumPy, Rust engine, juliacall,
  rustc), commit, seed, run parameters, per-tier availability, and the
  fastest-backend / parity index for each primitive.

A workstation run against a stale installed engine wheel honestly marks the
Rust tiers it lacks as unavailable; the CI column is the complete picture.
...
### Sparse Kuramoto CPU force and integrators

The sparse CPU route evaluates the same networked Kuramoto force over stored
SciPy sparse entries instead of materialising the dense `N x N` phase-difference
matrix. The committed artifact is `docs/benchmarks/sparse_kuramoto_cpu.json`,
generated by:
...
The benchmark uses bidirectional ring networks with `2N` stored edges. Measured
2026-07-09 with Python 3.12.13, NumPy 2.2.6, and SciPy 1.15.3:
...
* This is the supported path for classical sparse Kuramoto networks beyond the
  dense/small-system boundary. It keeps the dense Rust/Julia/Python chain
  unchanged for small dense matrices and adds explicit sparse SciPy input
  support for large rings, lattices, and graph-derived coupling matrices.
* The evidence is sparse classical CPU scaling only. It does not promote a
  quantum hardware claim or a broad performance claim.
...
* Rust is first at small-to-mid N for all three routes and wins the Hessian at N = 2048
  (130 vs 144 ms vs Julia). Julia's BLAS reduction edges the scalar energy ahead from N = 256
  (778 vs 888 µs) to N = 2048 (66 vs 84 ms), and edges the gradient at N = 2048 (118 vs
  131 ms); the chain keeps Rust first because it wins every small workload and the Hessian,
  and the large-N crossovers are small.
* Julia's matrix-marshalling cost dominates the gradient and Hessian at small N (169/28 µs at
  N = 4), so Rust and the vectorised NumPy floor lead until N ≈ 1024.
