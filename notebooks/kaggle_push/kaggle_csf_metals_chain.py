...
csf_oscillations = {
    ...
    "respiratory_variation": {"freq_Hz": 0.25,   "amplitude_mmHg": 1.5,  "source": "thoracic pressure"},
    ...
    "A_waves_plateau":      {"freq_Hz": 0.003,   "amplitude_mmHg": 50.0, "source": "5-20 min, pathological"},
...
for name, data in csf_oscillations.items():
    ...
    print(f"  {name:25s}: f={data['freq_Hz']:.4f} Hz (T={period:.1f}s), "
...
for metal, data in metal_ions.items():
    ...
    print(f"  {metal:10s}: conc={data['conc_mM']:.3f}mM, "
...
coupling_chain = [
    {"level": "Heart pump",        "freq_Hz": 1.2,    "coupling_type": "mechanical (pressure)",
     "medium": "blood", "coupling_to_next": "arterial pulsation"},
    {"level": "Arterial pulse",    "freq_Hz": 1.2,    "coupling_type": "pressure wave",
     "medium": "arterial wall", "coupling_to_next": "CSF pulsation"},
    {"level": "CSF pressure",      "freq_Hz": 1.2,    "coupling_type": "hydraulic",
     "medium": "CSF", "coupling_to_next": "skull bone stress"},
    {"level": "Skull piezoelectric","freq_Hz": 1.2,    "coupling_type": "mechano-electric",
     "medium": "bone (hydroxyapatite)", "coupling_to_next": "local E-field"},
    {"level": "Electric field",    "freq_Hz": 1.2,    "coupling_type": "electromagnetic",
     "medium": "extracellular fluid", "coupling_to_next": "ion channel gating"},
    {"level": "Na/K channels",     "freq_Hz": 500,    "coupling_type": "voltage-gated",
     "medium": "cell membrane", "coupling_to_next": "Ca2+ release"},
    {"level": "Ca2+ oscillation",  "freq_Hz": 0.1,    "coupling_type": "IP3/Ca2+ feedback",
     "medium": "cytoplasm", "coupling_to_next": "calmodulin activation"},
    {"level": "Enzyme cascades",   "freq_Hz": 1.0,    "coupling_type": "allosteric",
     "medium": "cytoplasm", "coupling_to_next": "kinase phosphorylation"},
    {"level": "CREB/gene expression","freq_Hz": 0.001, "coupling_type": "transcription factor",
     "medium": "nucleus", "coupling_to_next": "protein synthesis"},
    {"level": "Protein synthesis", "freq_Hz": 0.0001, "coupling_type": "ribosomal",
     "medium": "ribosome", "coupling_to_next": "cellular phenotype"},
]

print("\nHeart → Gene coupling chain:")
print(f"{'Level':25s} {'Freq (Hz)':>10s} {'Coupling':>20s} {'Medium':>25s}")
for link in coupling_chain:
    print(f"  {link['level']:25s} {link['freq_Hz']:>10.4f} "
...
print(f"Each link is a COUPLED OSCILLATOR with a characteristic frequency.")
print(f"The coupling type changes at each level (mechanical → electric → chemical → genetic).")
print(f"This IS the SCPN hierarchy — but with IDENTIFIED physical mechanisms.")
...
coordination_types = {
    ...
    "octahedral":   {"n_ligands": 6, "angle": 90.0,  "metals": ["Fe2+ (heme)", "Mg2+", "Mn2+", "Ca2+"]},
...
for geom, data in coordination_types.items():
    ...
    print(f"  {geom:15s}: {data['n_ligands']} ligands, angle={data['angle']}deg, "
...
print(f"Action potential = EXCITATION (phase slip in Kuramoto terms).")
print(f"Goldman-Hodgkin-Katz equation = MULTI-ION Kuramoto coupling!")
...
results["coupling_chain"] = {
    ...
    "coupling_types": ["mechanical", "hydraulic", "piezoelectric",
                       "electromagnetic", "voltage-gated", "chemical",
                       "allosteric", "transcriptional", "ribosomal"],
}
