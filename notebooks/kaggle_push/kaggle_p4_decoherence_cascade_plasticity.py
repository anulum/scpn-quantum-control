...
stages = {
    "1_molecular": {"timescale": (1e-15, 1e-12), "amplification": 1e3, "mechanism": "quantum coherence"},
    "2_protein": {"timescale": (1e-9, 1e-6), "amplification": 1e4, "mechanism": "conformational waves v~1000 m/s"},
    "3_cellular": {"timescale": (1e-3, 1e0), "amplification": 1e5, "mechanism": "Ca2+ oscillation gating"},
    "4_tissue": {"timescale": (1e0, 1e3), "amplification": 1e3, "mechanism": "gap junction coupling"},
}
...
add_finding("AMPLIFICATION_CASCADE", "Quantum-to-tissue amplification (Paper 4, p.66)", {
...
add_finding("PROTEIN_WAVE", "Conformational wave in protein (Paper 4, p.66)", {
...
add_finding("SYNC_PLASTICITY", "Coupling plasticity self-tunes network", {
...
add_finding("SYNC_MEMORY", "Memory encoded in coupling strengths", {
...
for n in range(1, 4):
    for m in range(1, 4):
        for sign in [1, -1]:
            ...
            if 0 < f_ghost < 200:
                ghost_freqs.append({"n": n, "m": m, "sign": "+" if sign > 0 else "-", "f_ghost_Hz": f_ghost})

add_finding("GHOST_SR", "Ghost stochastic resonance for missing frequencies", {
    "f1_Hz": f1,
    "f2_Hz": f2,
