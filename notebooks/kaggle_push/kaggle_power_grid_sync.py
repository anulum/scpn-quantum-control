...
sol = solve_ivp(swing_rhs, (0, 30), y0,
                t_eval=np.linspace(20, 30, 500), method='RK45', rtol=1e-6)
...
add_finding("GRID_SYNC", "Power grid steady-state synchronisation", {
...
add_finding("GRID_RENEWABLES", "Renewable noise degrades grid sync", {
...
for frac in line_fractions:
    ...
    def swing_cascade(t, y):
        delta = y[:N]
        omega = y[N:]
        ddelta = omega
        domega = np.zeros(N)
        for i in range(N):
            coupling = np.sum(adj_degraded[i] * np.sin(delta[i] - delta))
            domega[i] = (P[i] - D[i] * omega[i] - coupling) / M[i]
        return np.concatenate([ddelta, domega])
...
add_finding("GRID_CASCADE", "Cascading failure desynchronisation", {
...
for step in range(2000):
    ...
    def swing_h(t, y):
        ...
        for i in range(N):
            coupling = np.sum(adj[i] * np.sin(delta[i] - delta))
            domega[i] = (P_disturbed[i] - D[i] * omega[i] - coupling) / M[i]
        return np.concatenate([ddelta, domega])
    dy = swing_h(0, y_h)
    y_h += dt * dy
    if step % 10 == 0:
...
for step in range(2000):
    ...
    def swing_l(t, y):
        ...
        for i in range(N):
            coupling = np.sum(adj[i] * np.sin(delta[i] - delta))
            domega[i] = (P_disturbed[i] - D[i] * omega[i] - coupling) / M_low[i]
        return np.concatenate([ddelta, domega])
    dy = swing_l(0, y_l)
    y_l += dt * dy
    if step % 10 == 0:
...
add_finding("GRID_INERTIA", "Low-inertia grid more vulnerable to disturbance", {
    "max_freq_dev_high_inertia": round(float(max(freq_high_inertia)), 4),
    "max_freq_dev_low_inertia": round(float(max(freq_low_inertia)), 4),
    "vulnerability_ratio": round(float(max(freq_low_inertia) / max(max(freq_high_inertia), 1e-6)), 2),
