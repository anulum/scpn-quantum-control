...
def huygens_2(t, y):
    ...
    ddX = (-m * L * ((-g/L)*sin1 - b*dth1) * cos1
           - m * L * ((-g/L)*sin2 - b*dth2) * cos2
           - c * dX) / (M_p + 2*m*(1 - 0.5*(sin1**2 + sin2**2)))
...
sol2 = solve_ivp(huygens_2, (0, 60), y0_2, method='RK45', rtol=1e-8,
                 t_eval=np.linspace(50, 60, 2000))
...
add_finding("HUYGENS_2CLOCK", "Huygens' two clocks sync result", {
...
add_finding("METRONOME_N", "Sync vs number of metronomes", {
...
add_finding("METRONOME_MASS", "Platform mass controls coupling strength", {
...
add_finding("PHASE_SELECTION", "In-phase vs anti-phase selection", {
    "r_inphase_coupling": round(float(r_ip), 4),
    "mean_pair_diff_antiphase_rad": round(float(np.mean(phase_diffs_ap)), 4) if phase_diffs_ap else None,
